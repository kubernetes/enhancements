...
-->
# KEP-3693: Exclusion Rules for admission webhook configurations
...
I am a cluster operator and I want to suggest cluster critical resources all webhooks should exclude using these `exclusionRules` and `ValidatingAdmissionPolicy`
...
```
<<[UNRESOLVED @tallclair ]>>
@tallclair: I wonder if we want to streamline this API a bit more? I think it will be common to define the matching rules like:
...
And then want to exclude a few resource explicitly:

exclusionRules:
- objectNames: ["monitoring-server", "kube-proxy"]
namespaces: ["kube-system"]

In this case, I shouldn't need to repeat the apiGroups, apiVersions, operations & resources from the match rules. Leaving them blank should implicitly match everything.

@ivelichkovich: So how does that work if the user lists multiple rules though? how we would we know which set of the apiGroups/apiVersions/operations from which rule we want to inherit values from?
