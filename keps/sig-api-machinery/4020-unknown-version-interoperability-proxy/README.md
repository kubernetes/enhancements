...
- Case 3: If the resource state remains the same before and after the upgrade
    - If the resource state remains the same before and after the upgrade request can be served by any of the older/newer apiservers
...
1. Add a new header in client requests to specify the compatibility version of the client. Major clients to include for this change:
  1. KCM
  1. CCM
  1. Scheduler
  1. Client-go ???
...
2. compatibility version, binary-version of an apiserver stored as an annotation in apiserver-identity-lease
