...
Merged discovery is only supported for aggregated discovery endpoints, which require clients to use the aggregated discovery Accept headers. Requests for un-aggregated (legacy) discovery will always return local-only data and do not participate in peer merging. All discovery changes are implemented at the existing aggregated discovery endpoints (/apis and /api), with no new discovery endpoints being introduced.
...
![mvp_with_agg_discovery](https://github.com/kubernetes/enhancements/blob/master/keps/sig-api-machinery/4020-unknown-version-interoperability-proxy/assets/mvp_with_agg_discovery.png)
...
- `aggregator_discovery_merged_count_total` which is incremented everytime we construct a merged discovery response by merging resources served by a peer apiserver
