...
- [Proposal](#proposal)
  ...
  - [Risks and Mitigations](#risks-and-mitigations)
      ...
      - [Request Handler Blocking Indefinitely:](#request-handler-blocking-indefinitely)
      - [Impact When an HTTP/1x Connection is Hijacked:](#impact-when-an-http1x-connection-is-hijacked)
- [Design Details](#design-details)
  - [How We Manage Timeout Today:](#how-we-manage-timeout-today)
  - [Eanbling per-request Read/Write Deadline](#eanbling-per-request-readwrite-deadline)
  ...
  - [Monitor Post-Timeout Activity:](#monitor-post-timeout-activity)
...
- WATCH requests are outside the scope of this proposal, since currently we
...
- On start-up, the kube-apiserver will remove the `timeout` filter
  from the request handler chain.
- To every incoming non long-running request, the apiserver will 
  apply a read/write deadline.
- Replace the asynchronous mutating-request executor with one that 
  executes synchronously, in the same goroutine as the caller.
  
We also need to maintain parity with logs/metrics that are produced
by the `timeout` filter today for any request that times out.
...
With the `timeout` filter enabled today, it sends a `504 GatewayTimeout` response
or an `error` to the client, if the request handler it is executing is blocking indefinitely.

If the client does not enforce any timeout on its side, for example, not setting a
global `Client` timeout, or not setting a deadline to the request `Context`; the client
can hang forever waiting for a response from the server. This is certainly true for
HTTP/1x where [net/http does not execute the request handler in a new goroutine](https://github.com/golang/go/blob/b8ac61e6e64c92f23d8cf868a92a70d13e20a124/src/net/http/server.go#L2031-L2039).

> It's worth noting that by default `client-go` does not enforce any timeout globally.

With the removal of the `timeout` filter, we need to ensure that a request handler that
blocks on the server should never block a client that does not enforce timeout on its side.
...
```
Even though a read/write deadline is set, the client never receives any 
response or error from the server. We have opened an [issue](https://github.com/golang/go/issues/65526)
with the golang team to track this.

Mitigation Steps:
- a) If this is a bug, we can wait until the fix is in the net/http package.
- b) Otherwise, we can enable this feature only for http/2.0 request(s), all
  http/1x request(s) will continue to be served using the `Timeout` filter.

We also need to add unit tests to verify that a blocked request handler
does not block the corresponding client consequently:
```
...
##### Request Handler Blocking Indefinitely:

Once read/write deadline exceeds, the corresponding request handler goroutine will
finish its run by returning an error, or throwing a panic as soon as it uses the
`ResponseWriter` object by invoking `Write` or `FlushError`

This is in par with the `timeout` filter.


##### Impact When an HTTP/1x Connection is Hijacked:

The hijacker is responsible for managing the state of the hijacked
connection. Add a test to demonstrate that setting a read/write deadline before
...
Add test(s) to verify that with read/write timeout enabled, the above features
work as expected with hijacked connections.

## Design Details

### How We Manage Timeout Today:

Let's revisit how the `timeout` filter works today. Currently, the apiserver 
allocates a deadline to the `Context` of each non long-running request
inside the `WithRequestDeadline` filter:
```
...
- If a client specifies a zero timeout duration `{path}?timeout=0s`, then the value 
  of `--request-timeout` is used instead.
...
- a) client-facing: this is the [go routine that net/http creates](https://github.com/golang/go/blob/b8ac61e6e64c92f23d8cf868a92a70d13e20a124/src/net/http/h2_bundle.go#L5833) in order to run the request handler.
	
	client <-------  net/http (runHandler) <-------  (timeout handler)

- b) callee (timeout handler facing): this is the goroutine that the timeout 
  handler creates to execute the wrapped handler.

	timeout handler <------- (receiving channel) <------ callee goroutine

The client-facing goroutine waits via a receiving channel for the callee 
goroutine to finish, and it will wait until the deadline associated 
with the request `Context` exceeds. The following gist shows the essence
of the `timeout` filter. 
...
### Eanbling per-request Read/Write Deadline
...
- c) Remove the `timeout` filter from the handler chain, since `b` makes it redundant.

> Setting PerHandlerReadWriteTimeout=false must revert both `b` and `c`


We can utilize the deadline associated with the `Context` to set both the read and
write timeout for a given non long-running request.
...
> read/write deadline for anya given request, so for the initial implementation, we
...
`SetWriteDeadline` sets up a `time.AfterFunc` [1] with the given 
deadline, once the write deadline exceeds the stream is closed [2] with a `stream reset`
error, and the client sees the stream error immediately, although the request 
handler on the server might still be running after the timeout happens.
...
The `FlushError` method, on the other hand, is expected to return an
`i/o timeout` error immediately.
...
`SetReadDeadline` sets up a `time.AfterFunc` [1] with the given
deadline, once the read deadline exceeds, it closes the request `Body` with an
`i/o timeout` error [2]. 
...
Traditionally the `Flush` method of the underlying `ResponseWriter` object did
not return an `error`. With the introduction of per-request read/write deadline
the golang team has added a new version of `Flush` that returns an `error`, for
both http/1x and http/2.0
...
> With the use of `FlushError, the request handler can terminate by returning an error
...
Upon timeout, a request handler can finish its run by returning the error it encounters while:
...
Another option is to wrap the `ResponseWriter`, and the `Body` of the request, and panic on timeout
```
...
In addition to the `timeout` filter, the admission and rest logic of mutating-requests are executed 
asynchronously in their own goroutines by the [asynchronous finisher](https://github.com/kubernetes/kubernetes/blob/3a4c35cc89c0ce132f8f5962ce4b9a48fae77873/staging/src/k8s.io/apiserver/pkg/endpoints/handlers/finisher/finisher.go#L87-L122): 
...
### Monitor Post-Timeout Activity:

Upon timeout of a request, the `timeout` filter today provides with the following:

- increment the merics counter `request_post_timeout_total` by 1 for every 
  request that has timed out.
- print a line to the log

```
	err := fmt.Errorf("post-timeout activity - time-elapsed: %s, %v %q result: %v",
		time.Since(timedOutAt), req.Method, req.URL.Path, result)
	utilruntime.HandleError(err)
```

> After a request times out, the asynchronous finisher will spin up a new 
> goroutine and wait up to `5m` for the callee/child goroutine to return.
>
> The timeout filter does the same thing, but waits indefinitely until the
> callee goroutine returns.


With the `timeout` filter removed, we need to build a new mechanism to maintain parity.

##### Monitor Post-Timeout Activity using context.AfterFunc:

After the `Context` of a request times out and its handler has not returned yet,
it gets added to a global synchronized list.

We can setup a `PostStartHook` that goes through this list periodically and
determines how long a requst handler goroutine is active after the timeout
had happened.
...
type TimeoutTracker interface {
	// The request handler will invoke Enter after the request
	// times out via context.AfterFunc
	Enter(*RequestData)
...
// Inside the WithPerRequestTimeout filetr
func(w http.ResponseWriter, req *http.Request) {
	...
	stopFn := context.AfterFunc(req.Context(), func() {
		...
		data := &RequestData{StartedAt: started, DeadlineAt: deadline, RequestURI: req.RequestURI,
			RequestAuditID: "", HandlerDoneCh: handlerDoneCh}
		tracker.Enter(data)
	})
...
The `PostStartHook` goroutine will wake up every `5m`, and invoke `Sweep` which
will  perform the following steps on every `RequestData` item:

- if the `HandlerDoneCh` channel is closed, it indicates that the request handler
  returned after the context had timed out. The entry will be removed from the
  list and post-timeout activity will be reported in the metrics/logs.
- If an entry is resident for more than `1h`, we report it as hanging "forever"
  and remove it from the list, so the list does not grow infinitely.

> These two intervals can be tweaked as needed

Additionally, we can expose a debugging endpoint in the kube-apiserver that 
dumps a live snapshot of the post-timeout list.

### Impact on the Client

It's important to understand how the the client is impacted with this feature enabled; 
upon timeout of a request, what the client sees depends on the following factors:
...
If enabling this feature introduces a change in what the client observes, then it should be 
noted, and must be documented with a test. Let's outline the current behavior today, with
the `timeout` filter enabled:

When a request times out, the client today receives a `504 GatewayTimeout` only if the
request handler does not write to the underlying `ResponseWriter` object before
timeout occurs.

Moreover, if the client sets a timeout value in the request URI `{host/path}?timeout=10s` 
it will yield the same deadline for the request on both the client and the server. 
This is the case with `client-go` today. In this scenario, if a request times out, 
the client is most likely to receive a `context deadline exceeded` error due to
the fact that the client will see the `Context` of the client request be expired
well before the `504 GatewayTimeout` response from the server makes the round-trip
to the client.

Also, the client can not control when or whether the request handler writes to
the `ResponseWriter` object. So, today a client can not deterministically catch
the `504 GatewayTimeout` error from the server when a request times out.
...
The following goes through different scenarios, and for each we describe how the
server and client behaves differently when this feature is enabled.
...
- client: receives an `http.StatusGatewayTimeout` status code
- client: reading the `Body` of the `Response` object does not yield any error
- server: write to the `ResponseWriter` yields an `http: Handler timeout` errorg immediately

`PerHandlerReadWriteTimeout` - **Enabled**:
- client: http/2.0 client receives a stream reset error, on the other hand, http/1x 
  client receives a `local error: tls: bad record MAC` error.
- server: write to the `ResponseWriter` yields an `i/o timeout` error once the
  underlying buffer is full, so it may take a few `Write` invocations before the
  handler sees the timeout error.
...
`PerHandlerReadWriteTimeout` - **Disabled**:
- client: receives an http2 stream reset error, since the client-facing 
  goroutine in the `timeout` filter panics if the `ResponseWriter` object has
  already been written to.
- server: write to the `ResponseWriter` yields an `http: Handler timeout` error immediately


`PerHandlerReadWriteTimeout` - **Enabled**
- client: same as above (since net/http directly resets the http2 stream)
- server: write to the `ResponseWriter` yields an `i/o timeout` error once the
  underlying buffer is full.
  

*Observation* (with http/1x):

`PerHandlerReadWriteTimeout` - **Disabled**:
- client: receives an `EOF` error
- server: write to the `ResponseWriter` yields an `http: Handler timeout` error immediately
...
`PerHandlerReadWriteTimeout` - **Disabled**:
- client: receives a response with `http.StatusOK` code
- client: receives a stream reset error while reading the `Body` of the `Response`
- server: write to the `ResponseWriter` yields an `http: Handler timeout` error immediately
...
`PerHandlerReadWriteTimeout` - **Disabled**
- client: receives a response with `http.StatusOK` code
- client: receives an `unexpected EOF` error while reading the `Body` of the `Response`
- server: write to the `ResponseWriter` yields an `http: Handler timeout` error immediately


`PerHandlerReadWriteTimeout` - **Enabled**:
- client: receives a response with `http.StatusOK` code
- client: receives a `tls: bad record MAC` error while reading the `Body` of the `Response`
- server: write to the `ResponseWriter` yields an `i/o timeout` error once the
  underlying buffer is full.
...
`PerHandlerReadWriteTimeout` - **Disabled**:
- client: receives a response with `http.StatusOK` code
- client: receives an `context deadline exceeded` error while reading the `Body` of the `Response`
- server: write to the `ResponseWriter` yields either a `stream closed` or `http: Handler timeout` error.
...
Add unit tests to document and assert on the behavior of the per-request read/write
timeout using a net/http server. The following scenarios must be translated to
unit tests for both http/1x, and http/2.0:

- a) write deadline is set, after timeout occurs, we expect the `Write` method
  of the `ResponseWriter` object to return an `i/o timeout` error once its
  internal buffer is full (`FulshError` should be invoked).
- b) write deadline is set, let the write timeout occur, invoke `Write` once with
  small data (`w.Write([]byte("a")`, it is not expected to return error, 
  then invoke `FlushError`, it is expected to return an `i/o timeout` error immediately.
- d) write deadline is set, timeout occurs after the request handler writes to
  the `ResponseWriter` object using `Write`, but `FlushError` has not been invoked yet.
- e) write deadline is set, timeout occurs after the request handler writes and flushes
  the `ResponseWriter` object, using `Write` and `FlushError` in order.
- f) write deadline is set, the request handler keeps writing to the 
  the `ResponseWriter` object in a hot loop.
- g) read deadline is set, client does not send any content but keeps the 
  stream of the request `Body` open, read timeout is expected.
- h) read deadline is set, client sends some content and stops, but keeps the 
  stream of the request `Body` open, read timeout expected.
- i) read deadline is set, client request `Body` is empty, read timeout should have no effect
- j) client request has the same deadline as the read/write deadline on the server.

For each test above, assert on the following:
...
We need the above tests to understand how per-request read/write deadline behaves 
at the net/http level.


Next step is to write tests to verify whether the `WithRequestDeadline` filter
is setting the appropriate context deadline, and read/write timeout for the
request. The handler chain should consist of:
```
...
The Tests must exercise the following scenarios:

- request is a WATCH, long-running but not a WATCH, and non long-running 
...
In addition to the unit tests which verify the `WithRequestDeadline` unit, we 
should also write unit tests that exercise the complete handler chain of the
kube-apiserver.
```
...
not block the client, with both the feature enabled/disabled and for http/1x and http/2.0.
```
neverDoneCh := make(chan struct{})
func(w http.ResponseWriter, req *http.Request) {
	<-neverDoneCh
...
> differently and the client is impacted with this new feature enabled.
...
We exercise the following scenario with both http/1x and http/2.0 using client-go.
...
when the server times out its request. Instead, the client will receive an error 

NOTE: the client can not rely on the receipt of the `504 GatewayTimeout` HTTP status
code as it is not a deterministic error path on the server side.
