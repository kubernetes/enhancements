...
Kubernetes sometimes returns extremely large responses to clients outside of its local network,
resulting in long delays for components that integrate with the cluster in the list/watch controller
pattern. Kubernetes should properly support transparent gzip response encoding, while ensuring that the
performance of the cluster does not regress for small requests.

## Motivation

In large Kubernetes clusters the size of protobuf or JSON responses may exceed hundreds of
megabytes, and clients that are not on fast local networks or colocated with the master may
experience bandwidth and/or latency issues attempting to synchronize their state with the
server (in the case of custom controllers). Many HTTP servers and clients support transparent
compression by use of the `Accept-Encoding` header, and support for gzip can reduce total
bandwidth requirements for integrating with Kubernetes clusters for JSON by up to 10x and for
protobuf up to 8x.

### Goals

Allow standard HTTP transparent `Accept-Encoding: gzip` behavior to work for large Kubernetes
API requests, without impacting existing Go language clients (which are already sending that header)
or causing a performance regression on the Kubernetes apiservers due to the additional CPU necessary
to compress small requests.
...
An [issue](https://github.com/kubernetes/kubernetes/issues/112296) was opened to revisit the response compression by @shyamjvs (https://docs.google.com/document/d/1rMlYKOVyujboAEG2epxSYdx7eyevC7dypkD_kUlBxn4/edit?tab=t.0) which resulted in a few updates.

### 1.25

- [Add flag to disable compression for local traffic](https://github.com/kubernetes/kubernetes/pull/111507)

### 1.26

- [Reduce default gzip compression level from 4 to 1 in apiserver](https://github.com/kubernetes/kubernetes/pull/112299)
- [Add a "DisableCompression" option to kubeconfig](https://github.com/kubernetes/kubernetes/pull/112309)
- [Add --disable-compression flag to kubectl](https://github.com/kubernetes/kubernetes/pull/112580)
...
- `k8s.io/apiserver/pkg/endpoints/handlers/responsewriters`: `2025-08-23` - `68.9%`
...
Graduation criteria for Alpha needs to be decided on.
<<[/UNRESOLVED]>>
...
<<[UNRESOLVED optional short context or usernames ]>>
Graduation criteria for Beta needs to be decided on.
<<[/UNRESOLVED]>>

#### GA

- Any open question/concerns around throughput and performance have been resolved
  - Kubernetes compiled against a version of Go containing the fix for this issue: https://github.com/golang/go/issues/75532

<<[UNRESOLVED optional short context or usernames ]>>
Graduation criteria for GA needs to be decided on.
<<[/UNRESOLVED]>>
...
N/A - this feature has been in beta, default to on, since 1.16
...
Yes, the user can avoid sending requests with `Accept-Encoding` header.

###### What happens if we reenable the feature if it was previously rolled back?

APIServer can start gzip compressing again.
...
High CPU usage in APIServer, however, feature has been in beta (default to on) since 1.16
...
If a client is using a `Accept-Encoding` header with gzip compression
...
- [X] Other (treat as last resort)
  - Details: If a user adds `Accept-Encoding: gzip` to their request and get a gzipped response back, the feature is working
...
Yes, latency can change when using this feature
...
Yes, CPU can be used to compress API responses
...
This feature doesn't react any differently than before this feature was introduced.
...
* Kubernetes 1.8 added alpha implementation behind disabled flag
* Kubernetes 1.16 introduces more scope for beta implementation that addresses prior issues
