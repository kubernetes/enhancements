...
- [KEP-5073: Declarative Validation of Kubernetes Native Types With validation-gen](#kep-5073-declarative-validation-of-kubernetes-native-types-with-validation-gen)
  ...
  - [Proposal](#proposal)
    ...
    - [Introduce test framework](#introduce-test-framework)
...
*   Eliminate all hand written validation entirely in the long-term.  This will begin by iteratively removing more and more hand-written validation as the feature progresses to GA until all hand-written validations are removed entirely.
...
*   Introduce test framework and migration test utilities
    *   Ensures 100% equivalence between hand-written validation and declarative validation (de-risks migration problems)
...
A test framework and test utilities will be created as part of `validation-gen` to leverage the new centralized validation rules and to ensure validation consistency across hand-written and declarative validation rules.  See the [Test Plan](?tab=t.0#bookmark=id.a86sekfgbuen) section for more details.
...
TBD how we will verify that if a field is migrated we check that the associated validation_test.go has the proper equivalence test (eg: enforcement that a user made the necessary validation_test.go changes) This might be manually enforced in PR review? (or automated?)
...
15% of these are forbidden rules which primarily check which fields are allowed when a union discriminator is set. 10% are object name validations, and the remaining 10% are cross field validation checks, mainly mutual exclusion and some "transition rules" (e.g. immutability checks).


The remaining 70% of validation rules can be represented using JSON Schema value validations. `optional`, `format` and `enum` will be the most frequently used.  


Based on this analysis we believe that the proposed `validation-gen` design can completely implement all of the current hand-written validation logic via IDL tags with declarative validation replacement logic.  All of these validation types are supportable via generated go code + IDL tags.
...
Once validation is generated, it will be easy to opt-in::
...
The below rules are currently implemented or similar to an existing validator in the [validation-gen prototype](prototype )
...
<table>
  ...
  <tr>
   ...
   <td>
    <code>+k8s:min{Length,Properties,Items}</code>, <code>+k8s:max{Length,Properties,Items}</code>
   </td>
   <td>
    <code>min{Length,Properties,Items}</code>, <code>max{Length,Properties,Items}</code>
   </td>
  </tr>
  <tr>
   <td>
...
<table>
  ...
  <tr>
   ...
   <td>
    <code>+k8s:validationRule="{CEL-like expression}"</code>
   </td>
   <td>
  ...
  <tr>
   ...
   <td>
    <code>+k8s:validationRule="{CEL-like expression using oldSelf}"</code>
   </td>
   <td>
    <code>x-kubernetes-validations</code>
   </td>
  ...
  <tr>
   <td>
    special case: name format
   </td>
   <td>
    <code>+k8s:nameFormat={format name}</code>
   </td>
   <td>
    <code>x-kubernetes-validations </code>
   </td>
...
type ExampleSpec struct {
  ...
  // +k8s:subfield(name)=+k8s:+validationRule="!has(self.name) || self.name.isFormat('dns1123subdomain')",field="name"
...
Because the above validation case of formatting `metav1.ObjectMeta.Name`  is so common, we are also considering creating additional tags to make declaring the above rules more convenient:
...
```go
// +k8s:subfield(RestartPolicy)=+k8s:validationRule="!has(self.RestartPolicy)"
...
*   Break down the migration into smaller, manageable tasks. There are couple of options:
    ...
    *   Per Type: Migrate all validation rules for a single API. e.g. all validation rules of `ReplicationControllerSpec`.` `Df
