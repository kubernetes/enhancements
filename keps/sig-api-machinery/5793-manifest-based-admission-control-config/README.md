...
specific criteria (e.g., have a `platform.kubernetes.io/protected: "true"` label).
...
spec:
  ...
  validations:
  - expression: "!oldObject.metadata.labels.exists(k, k == 'platform.kubernetes.io/protected' && oldObject.metadata.labels[k] == 'true')"
...
Yes, integration tests verify correct behavior with feature gate enabled/disabled and with/without
