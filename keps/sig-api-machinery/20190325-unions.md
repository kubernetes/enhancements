...
- [Proposal](#proposal)
  ...
  - [Normalizing on server-side apply](#normalizing-on-server-side-apply)
...
- `// +discriminator` before a field means that this field is the
...
- `// +deduceDiscriminatorDeprecated` is a new marker that should only be
  used wuen adding a discriminator to existing types that previously lacked
  one. This field HAS TO be a string and MUST BE optional.
...
- `discriminator: <discriminator>` is set to the name of the discriminator
  field, if present,
- `deduce-discriminator: <boolean>` which defaults to false,
...
considered unset (and might get cleared).

This process works as follows:
- If the discriminator is deduced, at most one field must be set or an
  error is returned. The discriminator is then updated to match the field set.
- If the discriminator is not-deduced, all fields except the one
  matching the discriminator are cleared.

Later in the request handling pipeline, validation ensures:
- The set discriminator value is an allowed value (one of the recognized enum values)
- Required union members corresponding to set discriminators are present/valid.
...
That value is then kept in the final object and other fields are
cleared, whether the discriminator is deducible or not.
