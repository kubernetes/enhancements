...
Another area of interest is to avoid autoscaling if a pod is terminating.  In some cases a pod will be recreated where there is capacity and if there is no capacity autoscaling could be triggered.  This is not ideal if a pod is terminating and a user wants to wait until the pod is fully terminated.
...
ReplicaSet controller get a list of active pods.  Active pods usually mean pods that have not been registered for deletion.  In this KEP, we will consider terminating pods to be separate from active and failed.  This means that for cases where we track the number of pods, we should include a field that states the number of terminating pods.  

We will propose a new API fields in Deployments/ReplicaSets in this KEP.  
...
Generally, ReplicaSets utilize `FilterActivePods` in their reconciliation loop.  `FilterActivePods` gets a list of pods that are not terminating.  This KEP will include terminating pods in this list.
...
We will add an integration test that covers the following cases:

1) Deployment pods are terminating and if a user says they want to recreate only when fully terminating,
then this field allows that.
...
For enabling TerminatingPodsReplicaSetDeployments:
...
If a user terminates pods that are controlled by a deployment, then we should wait
...
This feature is closely related to the 3939-consider-terminating-pods-job.
