...
-->
# KEP-3328: Add queueName to the spec of Job
...
In Kubernetes world, Job plays an important role in batch scenarios, and generally it can be
managed by a queue or something similar for resource management, multi-tenant support or
special scheduling policies. But now we lack the native support of binding Jobs with a queue
...
calling out this feature. Currently a Kubernetes Job references to the kueue via the annotation,
which is not native.

So it's a good choice to support queueName in Job API, which helps to unify the usage for batch users.
...
-->
- Add a new field `queueName` to the spec of Job
...
nitty-gritty.
-->
Add a new field `queueName` to the spec of Job to unify the usage of batch users.
...
- `pkg/apis/batch/v1`: `2022-06-15` - `83.6%`
- `pkg/apis/batch/validation`: `2022-06-15` - `94.4%`
- `pkg/controller/job`: `2022-06-15` - `89.6%`
...
- Field `queueName` should be configured properly when enabling feature gate.
- Field `queueName` should be nil when disabling feature gate.
...
- Feature implemented behind a feature flag
- Initial tests completed and enabled
...
- What changes (in invocations, configurations, API use, etc.) is an existing
  ...
  - While the feature gate is enabled, `queueName` is default to nil.
  - While the feature gate is disabled, `queueName` is always nil.

- What changes (in invocations, configurations, API use, etc.) is an existing
  cluster required to make on upgrade, in order to make use of the enhancement?

  - Enable the feature gate, and specify the `queueName`.
...
- [x] Feature gate (also fill in values in `kep.yaml`)
  - Feature gate name: JobQueueName
...
-->
People can set the `queueName` of Job and verify it in the Job spec.
...
-->
- 2022.06.15: KEP proposed for review, including motivation, proposal, risks,
