...
This integration follows the patterns established by the Job controller integration
([KEP-6089](https://github.com/kubernetes/enhancements/tree/master/keps/sig-scheduling/6089-was-controller-apis)),
using the reusable `workloadbuilder` library and `scheduling.k8s.io/v1alpha3` building blocks.
...
patterns established by the Job controller integration in KEP-6089 and aligning with the broader
...
- Mutable `spec.scheduling` post-creation (toggle on/off, flip gang/basic, or change topology
  constraints); scheduling configuration is immutable for Alpha.

- User-set `gang.minCount` — unlike the design document which allowed user-specified minCount,
  this KEP derives it from `spec.replicas` (matching the Deployment integration pattern).
  Partial-gang semantics (`minCount < replicas`) are deferred to Beta.
...
// gang minCount == replicas. Requires podManagementPolicy: Parallel for gang;
...
`StatefulSetSchedulingConfiguration` embeds the same `scheduling.k8s.io/v1alpha3` building-block
types as `DeploymentSchedulingConfiguration` ([KEP-6276]), but omits `ResourceClaims` —
StatefulSets manage persistent storage through `volumeClaimTemplates`, and PodGroup-level DRA
claims do not interact with that mechanism. The field may be added in a future release if a
...
type StatefulSetSchedulingConfiguration struct {
    ...
    // is valid. The controller derives minCount from spec.replicas;
    // a user-supplied minCount is rejected at admission.
    // +optional
    // +k8s:optional
    // +k8s:update=NoSet
    // +k8s:update=NoUnset
...
1. **User-set `gang.minCount` is rejected**: Like the Deployment integration ([KEP-6276]),
   `minCount` is always derived from `spec.replicas` by the controller. If `minCount > replicas`,
   the gang can never be satisfied; if `minCount < replicas`, true partial-gang semantics require
   multiple PodGroups which is deferred to Beta. A user-supplied `minCount` value is rejected at
   admission.

2. **Mutable minCount on PodGroup**: Per KEP-5832, `minCount` on the standalone `PodGroup` object
...
5. **Feature gate dependency**: This feature depends on the `GenericWorkload` feature gate
...
| `spec.replicas` | `podGroupTemplates[0].schedulingPolicy.gang.minCount` | All replicas scheduled together |
...
Each pod created by the StatefulSet receives a scheduling group reference:

```yaml
# Pod spec with schedulingGroup reference
spec:
  schedulingGroup:
...
is requested and `minCount` is not explicitly specified by the user.
...
**Open question**: Removing a partition after it has been set requires updating `workloadRef` on
existing pods, which is currently immutable. The only known solution is pod recreation.
...
StatefulSet Created (podManagementPolicy: Parallel, scheduling.policy.gang)
  │
  ├─→ Controller creates Workload (static template)
  │
  ├─→ Controller creates PodGroup (runtime, minCount = replicas)
  │
...
immutability of `spec.schedulingGroup.podGroupName` on pods.

### Enabling Gang Scheduling

Gang scheduling is enabled via the `spec.scheduling` field on the StatefulSet, following the
controller-as-compiler pattern from [KEP-6089] and [KEP-6276]:
...
The user sets `gang: {}` — the controller derives `minCount = *spec.replicas`. A user-supplied
`minCount` is rejected at admission. Workload and PodGroup names are deterministic:
`<statefulset-name>` for both, matching the StatefulSet's own name.

### Admission Validation

Validation follows the same three-layer approach used across all WAS-integrated controllers
([KEP-6276], [KEP-5547]), but the StatefulSet layer adds rules specific to `podManagementPolicy`
...
2. **Hand-written StatefulSet validation** — cross-cutting rules that span fields DV cannot
   ...
   - **OrderedReady + Gang conflict**: Reject if `spec.scheduling.schedulingPolicy.gang` is
     ...
     default). This is the StatefulSet-specific equivalent of the Deployment KEP's
     `maxSurge < replicas` rejection — both prevent structurally guaranteed deadlocks.
     ```
...
- `pkg/apis/apps/validation`: Coverage for admission validation rules.
  ...
  - Reject user-set `gang.minCount` (must be nil — derived from replicas)
...
- Scale-up with Workload recreation: verify all pods rescheduled together with new MinCount
- Scale-down with Workload recreation: verify correct pod termination and Workload update
...
- Gather feedback from early adopters on the drain-and-recreate scaling behavior
...
Unlike the Deployment integration ([KEP-6276]) where scheduling objects are owned by the
ReplicaSet and pruned via `revisionHistoryLimit`, the StatefulSet controller owns Workload and
PodGroup objects directly. This means:
...
The StatefulSet ownership model is simpler than the Deployment case: there is no bootstrap-own →
reown sequence because the StatefulSet directly owns the Workload and PodGroup (no ReplicaSet
indirection). This eliminates the Deployment's crash window between bootstrap ownership and
reown, but it also means scheduling objects have no independent lifecycle — they exist exactly as
long as the StatefulSet does.
...
- **kube-controller-manager**: Stops compiling Workload and PodGroup objects. Existing scheduling
  objects become inert but remain in the cluster because they are owned directly by the
  StatefulSet (unlike the Deployment case, there is no ReplicaSet pruning that would
  garbage-collect them). They are cleaned up when the StatefulSet is deleted, or they can be
  manually removed.
...
executing. Workload and PodGroup objects become inert but persist in the cluster (they are owned
by the StatefulSet, not by a prunable ReplicaSet). New pods created after rollback lack
...
- `statefulset_workload_creation_errors_total`: Persistent errors creating Workloads indicate a
  problem with the integration.
- `statefulset_workload_recreation_total`: Unexpectedly high recreation rates may indicate a
  reconciliation loop.
- `scheduler_permit_wait_duration_seconds`: Unusually long permit wait times for StatefulSet pods
  may indicate gang requirements cannot be satisfied.
...
- Check for `Workload` objects with `spec.controllerRef.kind: StatefulSet`:
  ...
  ```
- The metric `statefulset_workload_creation_total` is non-zero when the feature is actively used.
...
- [x] Events
  - Event Reason: `WorkloadCreated` — emitted on the StatefulSet when a Workload is successfully
    created.
  - Event Reason: `WorkloadRecreated` — emitted when a Workload is recreated due to scaling.
...
- 99.9% of Workload creation attempts succeed within 5 seconds of StatefulSet creation.
- 99% of gang-scheduled StatefulSet pods are bound within the same scheduling cycle once MinCount
  is satisfiable.

###### What are the SLIs (Service Level Indicators) an operator can use to determine the health of the service?

- [x] Metrics
  - Metric name: `statefulset_workload_creation_duration_seconds`
    - Components exposing the metric: `kube-controller-manager`
  - Metric name: `statefulset_workload_creation_errors_total`
    - Components exposing the metric: `kube-controller-manager`
  - Metric name: `statefulset_workload_recreation_total`
    - Components exposing the metric: `kube-controller-manager`

###### Are there any missing metrics that would be useful to have to improve observability of this feature?

Gang satisfaction latency from the scheduler side (time from first pod entering Permit to gang
being satisfied) would be useful but is tracked as part of the core Workload API metrics in
KEP-4671, not specific to this integration.
...
Yes. The StatefulSet controller makes direct API calls (no informer in Alpha) for each StatefulSet
with `spec.scheduling` set:
- `GET Workload` + `CREATE Workload` — 1 per StatefulSet creation
- `GET PodGroup` + `CREATE PodGroup` — 1 per StatefulSet creation
...
objects (direct API calls in Alpha). The gang scheduling Permit phase may add additional latency
...
Minimal for Alpha (opt-in only, no informer). Per-StatefulSet overhead is 2 additional small
objects in etcd (~500 bytes each) and 2 GET + 2 CREATE API calls per new StatefulSet.
...
| Gang cannot be satisfied | Cluster lacks resources for MinCount pods | Pods remain in Permit phase; `scheduler_permit_wait_duration_seconds` increases | Scale cluster or reduce MinCount |
...
- [KEP-6276: Integrate Workload with Deployment](https://github.com/kubernetes/enhancements/issues/6276) — Deployment controller integration (peer KEP, in progress)
...
- [OCPNODE-4667](https://redhat.atlassian.net/browse/OCPNODE-4667) — OpenShift Node tracking issue
