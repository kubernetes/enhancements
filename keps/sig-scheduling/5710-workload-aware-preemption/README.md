...
- expose the information about divergence in the API by introducing a new `Conditions` field
  ...
  be set by iehter kube-scheduler or a new workload-controller whenever it observes pods
...
- making `pod.Spec.PriorityClassName` and `pod.Spec.Priority` mutable fields and having a new
  ...
  workload object (e.g. Job) which would introduce a simimlar level of confusion to users.
...
preempt low priority large workload elsewhere, preemting which would be just enough for us).
