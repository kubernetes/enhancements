...
**Approach:** Add status to ResourceClaim showing total pool capacity and availability

**Pros:**
- Fits with existing ResourceClaim model
- No new controller needed

**Cons:**
- ResourceClaims are namespaced, so capacity info would be duplicated across namespaces
- Doesn't help with cross-namespace visibility
- Doesn't solve the core problem: seeing available resources before creating a claim
- Would require either duplicating capacity info or cross-namespace references

**Decision:** Rejected - doesn't solve the visibility problem

### Alternative 2: New cluster-scoped ResourcePool object

**Approach:** Create a new ResourcePool CRD that aggregates information across ResourceSlices

**Pros:**
- Clean separation of concerns
- Could include additional pool-level metadata

**Cons:**
- Introduces a new API type
- More complex for users (ResourceSlice + ResourcePool)
- Drivers would need to manage both types
- Doesn't integrate with existing ResourceSlice API

**Decision:** Rejected - unnecessary complexity, ResourceSlice status is sufficient

### Alternative 3: Metrics-based visibility only

**Approach:** Expose availability only through metrics, not API status
...
- Metrics may not be available in all clusters
- Doesn't provide real-time API-level visibility
- Cannot correlate with specific ResourceClaims

**Decision:** Rejected - doesn't meet user experience goals

### Alternative 4: Aggregated API server for DRA

**Approach:** Build a separate API server that aggregates DRA information

**Pros:**
- Very flexible
- Could provide advanced querying

**Cons:**
- Massive engineering effort
- Requires deploying and maintaining separate component
- Overly complex for the problem at hand

**Decision:** Rejected - disproportionate to the need

### Alternative 5: External controller with custom CRD

**Approach:** Allow DRA drivers to create their own status CRDs

**Pros:**
- Flexible for driver-specific needs
- No changes to core Kubernetes

**Cons:**
- Inconsistent across drivers
- No standard kubectl integration
- Users need to learn different APIs per driver
- Doesn't solve cross-driver visibility

**Decision:** Rejected - inconsistent user experience
