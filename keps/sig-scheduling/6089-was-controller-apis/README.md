...
and the shared `workloadbuilder` library in `k8s.io/component-helpers`, validated via the core `Job` API (`batch/v1`) integration.
...
In this alternative model (which was evaluated in JobSet but rejected in favor of Option A), leaf-level scheduling
...
This model was rejected in JobSet because:
...
- **Delegated Management (Nested / Chained Controllers):**
  ...
  `TrainJob` can patch the `JobSet` template embedded in its `ClusterTrainingRuntime`, but each
  `ReplicatedJob` has a single `JobTemplateSpec` shared by all of its generated `Job` replicas.
  When those replicas require distinct leaf `PodGroup`s, `TrainJob` cannot set a different
  `job.spec.template.spec.schedulingGroup.podGroupName` for each generated `Job`. In contrast,
  `JobSet` constructs each child `Job` and can set the reference to the corresponding `PodGroup`
  while doing so.

  Therefore, a wrapper controller like `TrainJob` cannot directly bind each generated `Job`
  replica's pods to a distinct, centrally created leaf `PodGroup`. Instead, it must **delegate**
...
* **Graduation to Beta (v1.38) via Direct-to-v1 Promotion:** When the composed fields are promoted to Beta
  ...
  fields inside `batch/v1.JobSpec` to reference the `v1` types.
