...
- This feature does not respect PodDisruptionBudget during the eviction.
...
As the scale grows, I added more nodes for user B, and all existing nodes are only available to user A.
I can simply delete the "userB=allow" label from the existing nodes to re-schedule all pods of user B to these new nodes.
...
- It does respect terminationGracePeriodSeconds.
...
- descheduler
  ...
  If kubernetes is going to implement this feature, descheduler can no longer maintain the `RemovePodsViolatingNodeAffinity` policy. And if users only rely on this policy for rescheduling, they can remove the descheduler from the system and migrate to kubernetes to reduce the complexity of the system. No services will be unavailable during the migration process.
