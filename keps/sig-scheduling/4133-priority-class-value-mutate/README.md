...
# KEP-4133: Allow priority class values to be updated
...
- Allow mutating priorityClass value
...
The priorityClass validation logic in the API server, what we need to do is relax the validation
of the priorityClass value field which is currently immutable to be mutable.

A new featuregate flag called `AllowPriorityClassUpdates` will be added. By enabling this flag, 
...
- `k8s.io/kubernetes/pkg/apis/scheduling/validation`: `02/03/2023` - `86.7%`
...
- Test that `priorityClass` value field Could be mutated when `AllowPriorityClassUpdates` featuregate enable.
