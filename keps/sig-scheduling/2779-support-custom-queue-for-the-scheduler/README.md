...
2. The registered custom queue is recorded by schedulerOptions which is also
used to record the registered custom plugins.
  ...
  + customQueue         SchedulingQueue
  }
  ```

3. The custom queue then will be recorded by Configurator
  ```
  ...
  +   customQueue:         options.customQueue,
  }
  ```

4. In function (c *Configurator) create(), pass the custom queue to the scheduler
instead of creating the current queue.
  ```
  + podQueue := c.customQueue
