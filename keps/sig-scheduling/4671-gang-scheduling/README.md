...
The `Workload` object will allow kube-scheduler to be more aware that pods are part of workloads with complex internal structure.  Those workloads include builtins like `Job` and `StatefulSet`, and custom workloads, like `JobSet`, `LeaderWorkerSet`, `MPIJob` and `TrainJob`. All of these workload types are used for AI training and inference use cases.
...
- Ensuring that `Workload` API will be usable both for both built-in and third-parth workload controllers and APIs
...
- Address resource contention between different scheduler.
...
1. Ensure that pods being part of a gang are not scheduled if all pods belonging to it can't be scheduled.
...
4. Avoid deadlock scenario when multiple workloads are being scheduled at the same time by different
   schedulers.
5. Avoid preemptions of already running pods if these will not allow to schedule higher priority gang
   anyway.
...
For `Beta`, we want to also touch requirements (2) and (3). In order to achieve it, we want to extend
the scheduling framework with a new dedicated phase (tentatively called Workload). In that phase,
...
After that, pods will go through other phases (including Filter and Score) with a nomination as a form
of validation the proposed placement and execution of this placement decision.

While we will not target addressing "optimal enough" part of requirement (2), we will prove that we
can process all pods together. The single scheduling cycle and blocking resources will address the
requirement (3).
