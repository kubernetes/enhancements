...
- Support an aggregated resource consumption request. 
  ...
  However, a user may want an aggrated amount of resources which can come from a single or multiple shared device. 
...
This KEP mainly focuses on the request with selection of shared device (`device.shared == true`).
...
Story|Consumable Capacity|Selector|Resource Request|AllocationMode|Constraint|Context
---|---|---|---|---|---|---
1|no|no|no|Exact(1)|no|network
2|no|no|no|Exact(2)|yes|network
3|no|yes|no|Exact(1)|no|network
4|yes|no|no|Exact(1)|no|network
5|yes|no|yes|Exact(1)|no|network
6|both|no|yes|Exact(1)|no|network
...
A user omits the capacity request. 
...
If the cliam policy field is not empty. Either one of the policies must be defined with default value.
...
- shared device are handled separately from the other device.
  ...
  - After `isSelectable` check, check whether the (rounded-up) requested capacity is larger than capacity deducted by `aggregatedCapacity` and `allocatingCapacity`.
    ...
    [Implementaion of consumable capacity related functions](https://github.com/sunya-ch/kubernetes/blob/kep-5075/staging/src/k8s.io/dynamic-resource-allocation/structured/consumable_capacity.go)
...
- Extension of TestAllocator in `structured` module. 
  ...
  |one-shared-device-with-distinct-constraint|yes|2 consumable|0|yes|1+1, distinct|no
  |two-shared-devices-with-distinct-constraint|yes|2x1 consumable|0|yes|1+1, distinct|yes
