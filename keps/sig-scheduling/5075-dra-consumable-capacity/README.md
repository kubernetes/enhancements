...
- When a driver updates the request policy, the behavior of resource claims changes.
  ...
  **Mitigation:**
  - The validation process should either warn users or block updates to the request policy of devices already in use.
...
If users do not specify a capacity request, the default consumed value will be the device’s full capacity or the default value specified by the request policy.
