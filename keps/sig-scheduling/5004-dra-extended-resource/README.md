...
3. **Upgrade Testing** (Feature Gate: ON → ON, version n-1 → n)
   - **Pre-upgrade state**: Cluster running version n-1 with DRAExtendedResource feature enabled
     - Workloads using DRA-backed extended resources are running
     - DeviceClass with extendedResourceName exists
     - Special ResourceClaims created by scheduler exist
   - **Upgrade**: Upgrade cluster from version n-1 to version n while keeping DRAExtendedResource feature gate enabled
...
4. **Disablement Testing** (Feature Gate: ON → OFF)
...
5. **Enablement→Disablement→Enablement Path**
   - **Initial enablement**: OFF → ON (as described above)
   - **Disablement**: ON → OFF
...
This comprehensive testing approach ensures the DRAExtendedResources feature is robust across all enablement, disablement, upgrade, and rollback scenarios before promoting to beta and GA.
