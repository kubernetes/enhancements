...
- shared counter allocation attempts/success/failure by reason
- consumed vs available shared counter capacity per set (cardinality constraints permitting)
...
- Splits the `Counter` struct into two, which is an API-level change to an existing type.
