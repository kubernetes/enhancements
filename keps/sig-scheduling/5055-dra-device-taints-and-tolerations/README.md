...
but not when they are in different objects, so instead the
consumers need to handle this for device taints at runtime by checking that all
of them are tolerated without assuming uniqueness by key and effect.
...
taints through DeviceTaintRule objects.
