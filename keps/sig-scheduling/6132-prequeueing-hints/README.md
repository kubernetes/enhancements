...
pods referencing them. An async event queue decouples ResourceClaim
event handling from the informer thread.
...
- Decouple ResourceClaim event handling from the informer thread
  to prevent blocking.
...
| Async event queue introduces ordering issues | Uses workqueue with deduplication; ordering within a single event type is preserved. |
...
event. The framework unions their pod lists. If any plugin signals
"all pods", the framework evaluates all pods.
...
Currently, ResourceClaim informer event handlers call
`MoveAllToActiveOrBackoffQueue` synchronously. This acquires
the scheduling queue lock on the informer goroutine, which can
block the informer from delivering other events.

This KEP decouples the two by introducing a workqueue between
the informer and the scheduling queue:

- The informer handler enqueues a lightweight key (claim
  namespace/name) into a dedicated workqueue.
- A separate worker goroutine drains the workqueue and calls
  `MoveAllToActiveOrBackoffQueue` (which now benefits from
  `PreQueueingHintFn` to avoid the O(N) scan).

This prevents informer thread starvation under high claim
event volume.
...
- When disabled: `PreQueueingHintFn` is ignored, all pods are
  evaluated on every event (existing behavior). The async
  event queue is also disabled.
...
- [x] Async event queue for ResourceClaim processing
...
- 2026-05-08: Initial PR with OwnerReference-based approach
  ([#138916](https://github.com/kubernetes/kubernetes/pull/138916))
- 2026-05-13: Added deallocation handling
- 2026-05-27: Rebased, CI green
- 2026-06-09: Revised to pod informer index approach per reviewer
  feedback
- 2026-06-10: KEP updated: removed code blocks, added explicit
  boolean return, clarified async queue motivation, added PRR file
