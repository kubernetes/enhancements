...
can be inmproved, and we will consider those opportunities. The implication of
...
<<[UNRESOLVED where the code lives]>>
...
and global tags (%TAG !...) may be simplified when rendered to KYAML, as long
as the resulting unmarshaled object would be identical.
...
map has a line-comment it will be rendered after the closing brace (since
this is the only place it will be found upon re-reading).
...
<<[UNRESOLVED quoted keys]>>
Should we output field names and map keys:
a) As described above, with only "non-obvious" values quoted?
b) Always quoted (more like JSON)?
<<[/UNRESOLVED>>
...
users can provide vanilla YAML anyywhere that can accept KYAML.
