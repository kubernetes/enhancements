...
4. Changing the YAML library used by Kubernetes.
...
The simplest thing to do is to write a new implementation of
k8s.io/cli-runtime/pkg/printers.ResourcePrinter (in
staging/src/k8s.io/cli-runtime/pkg/printers). However, this isn’t the most
obviously reusable place for it.

A more ecosystem-friendly solution is to write a new function or package under
sigs.k8s.io/yaml](sigs.k8s.io/yaml) which can be used by other tools.
...
when fed back into the tool, should not be modified again.
...
render Go objects and re-render YAML or JSON input. That extra conversion is
...
as compound non-string keys, and things that Go's JSON encoder quietly converts
to strings, such as numbers.

Today we have forked go-yaml.v2 AND go-yaml.v3 into sigs.k8s.io/yaml.  While
KYAML is an output format, our tooling goals require that we get access to a
YAML AST.  Fortunately, go-yaml.v3 offers a way to decode YAML into such an
AST. If we ever change YAML libraries (e.g.
[https://github.com/goccy/go-yaml](https://github.com/goccy/go-yaml)), we will
need to rewrite this code.

### Comments

When reformating existing YAML with comments, we will attempt to place comments
...
This disambiguates it from JSON, since both KYAML and JSON begin with an
opening curly-brace.  Kubernetes versions 1.33 and higher can handle KYAML
without the header, but it is needed for versions before that.
...
Structs are rendered with curly-braces, and each field on a line of its own. If
a struct has a line-comment it will be rendered after the closing brace (since
this is the only place it will be found upon re-reading).
...
will be rendered as inline fields.
...
The `yaml` struct tag will be ignored.

##### Maps

Maps are rendered with curly-braces, and each key on a line of its own. If a
map has a line-comment it will be rendered after the opening brace.

Because we marshal Go objects to JSON, map keys must be strings. This is more
restrictive than plain YAML. Reformatting YAML with non-string keys into KYAML
will fail.

Keys are sorted and are not quoted unless they are non-trivial or are
type-ambiguous (e.g. `no`)
...
Slices and arrays are rendered with square-braces. If a struct has a
line-comment it will be rendered after the opening brace.
...
Con: Requires weird EOL handling (`\n\` meaning a newline, plus a
     line-continuation which  prevents the fold from creating a single leading
     space on the next line)
Con: Subtle handling of leading whitespace (all dropped unless escaped, same as option 3)

###### Variation A. Minimal impact, misaligned vertical

This variation escapes leading whitespace and does nothing special when
rendering lines without leading whitespace.  The result is that vertical
...
###### Variation B. More impact, aligned vertical

This variation escapes leading whitespace and adds an extra do-nothing space
when rendering lines without leading whitespace.  The result is that vertical
alignment is preserved, but the indent appears off-by-one.
...
Con: Requires weird and surprising EOL handling (double-newline being a
     directive to YAML to add a single newline to the output)
Con: Harder to read than option 2 (vertical alignment in particular)
Con: Subtle handling of leading whitespace (all dropped unless escaped, single
     newline inserts a space)
...
element, unless the closing brace is cuddled.  As mentioned elsewhere, the
...
Paired braces are always cuddled, unless the next item has a head-comment or
...
```
list: [{
  foo: “abc”, # This is a line-comment
  bar: 123,
}, {
  foo: “def”,
  bar: 456,
...
Partially cuddled, with a head-comment.
```
list: [
...
Partially cuddled, with a foot-comment.
```
list: [{
  foo: “abc”,
  bar: 123,
...
<<[UNRESOLVED uncuddle if comments]>>
The "mixed" cuddling demonstrated above is a little odd.  As an alternative, we
could detect if any item has head- or foot-comments and just uncuddle all
braces in that list.

Or we could style lists with an extra indent like:
```
list: [{
    foo: “abc”,
    bar: 123,
...
No affordance is made to render short lists, maps, or structs on a single line.
This is possible but was deemed unnecessary.
...
This KEP does not propose a new feature gate.  KYAML is opt-in and is entirely
client-side.
...
they are sure their clients are updated, but there is little risk of breakage
from ad hoc use.
...
We could tell users to use JSON with comments and a JSON minifier tool to strip
those out before feeding to kubernetes.

We could “ban” the use of raw JSON (except to the API) and instead use
JSON-with-comments (via YAML parsing) in all configuration logic.  This doesn’t
solve for kubectl output.

We could adopt [HJSON](https://hjson.github.io/) or
[JSONC](https://www.npmjs.com/package/jsonc) or [JSON5](https://json5.org/).
