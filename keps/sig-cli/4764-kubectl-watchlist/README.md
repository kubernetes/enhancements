...
for user to refactor just "a few commands" into "a batch of complex code". So we are seeking an optimization for `kubectl`. With this optimization, all users of `kubectl` can get free performance improvement.
...
Existing `kubectl` subcommands like `get`, `describe` should be able to use a _watch list_ to fetch collections (list resources), rather than directly list resources page by page.
