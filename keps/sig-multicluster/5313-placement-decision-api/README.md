...
metadata:
  ...
  labels:
    ...
    multicluster.x-k8s.io/decision-key: "argocd/app" # optional correlation label
...
**Example Argo CD as consumer**
- An ApplicationSet needs a set of target clusters.
- Two viable patterns:
  1. *Label selector*: ApplicationSet (or a companion controller)
  watches `PlacementDecision` with label `multicluster.x-k8s.io/decision-key=<app-namespace>/<app-name>`,
  and creates one Application per `ClusterDecision`.
  2. *Deterministic naming*: The controller reads `PlacementDecision` objects named after the Application (`<appname>-<index>`),
  aggregates slices, and creates one Application per `ClusterDecision`.
