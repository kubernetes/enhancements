 ...
 * The rollout controller defines the rollout plan and creates a `PlacementDecision` keyed to the release.
 * It updates the `PlacementDecision` as health gates pass to expand or contract the target set.
 ...
 * A scheduler publishes one `PlacementDecision` for the placement key owned by the initiator.
