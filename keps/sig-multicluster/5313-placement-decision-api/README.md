...
- **Placement key**: An correlation string to associate the placement request/decision with the workload.
...
4. Scheduler/PlacementController writes one or more `PlacementDecision` slice objects,
   all correlated via the same `multicluster.x-k8s.io/decision-key`.
4. A new controller watches/gets those `PlacementDecision` objects and finds the corresponding `ApplicationSet` by placement key.
5. The controller updates the corresponding `ApplicationSet`'s generator spec based on the `PlacementDecision` object.
6. ArgoCD controller applies or removes workload to match the generator spec.
...
2. Fleet Admin creates a `MultiKueue` configuration whose `DispatcherName` field is the name of muticluster scheduler on the hub cluster. This indicates MultiKueue to delegate the placement decision to the [external scheduler](https://kueue.sigs.k8s.io/docs/concepts/multikueue/#external-custom-implementation).
...
5. Scheduler/PlacementController writes one or more `PlacementDecision` slice objects, all correlated via the same `multicluster.x-k8s.io/decision-key`.
6. A new controller watches/gets those `PlacementDecision` objects and finds the corresponding kueue `Workload` by placement key.
...
3. Consumer chooses the usage pattern (no API markings required):
   - One-shot: Read the `PlacementDecision` once (by the workload's placement key), act and ignore future updates.
...
// Optional: Label that links all slices to the same decision key.
// If set, all slices belonging to one logical decision must share the same value.
...
metadata:
  ...
  namespace: argocd
  labels:
    multicluster.x-k8s.io/decision-index: "0"                           # optional sorting label
    multicluster.x-k8s.io/decision-key: "argocd-app-placement-decision" # optional correlation label
...
**Label selector**
- Producer sets the same `multicluster.x-k8s.io/decision-key=<decision-key>`
label value on all slices for one logical decision (optional but recommended when multiple slices are expected).
- Consumer lists/watches with `labelSelector=multicluster.x-k8s.io/decision-key=<decision-key>`
in its namespace.
- If ordering matters and results span multiple slices,
produer should set `multicluster.x-k8s.io/decision-index=<0..N>` and consumer can sort by that label.
...
* To correlate slices, producers may:
  * set the same `multicluster.x-k8s.io/decision-key=<decision-key>` label on all slices.
  * use a deterministic naming scheme with a common base name and `<slice-index>`.
...
- **Create**: The scheduler creates one or more slices with the list of clusters in the decision.
  ...
  - **Label selector** correlation: set `multicluster.x-k8s.io/decision-key=<decision-key>` on every slice,
  and set `multicluster.x-k8s.io/decision-index` when order matters.
...
- **Delete**: When a scheduling decision is no longer required.
  ...
  Examples of when a decision is no longer needed:
...
* Argo CD's generator logic computes targets and writes `PlacementDecision`s.
* An Argo CD controller reads the `PlacementDecision` and creates Applications for each target cluster.
* Benefit: internal decoupling generators focus on "where" and consumers focus on "apply".

#### Story 5: Multiple consumers fan-out
* A scheduler publishes one `PlacementDecision`.
* Multiple consumers (GitOps, DR, etc.) act on the same data.
* The outcome is consistent targeting across tools with no duplicate placement logic.
