...
and `ServiceImport`, along with a `PreparedSelector` API for
...
type PreparedSelector interface {
    // SelectClusters returns the subset of candidates that match.
    SelectClusters(candidates []Cluster) ([]Cluster, error)
}
...
// across evaluations.
func PrepareSelector(selector ClusterSelector, importer Cluster) (PreparedSelector, error)
```
