...
The `ClusterProperty` CRD will support two specific properties under the well known names `cluster.clusterset.k8s.io`, `network.clusterset.k8s.io` and `clusterset.k8s.io`. Being "well known" means that they must conform to the requirements described below, and therefore can be depended on by multi-cluster implementations to achieve use cases dependent on knowledge of a cluster's ID or ClusterSet membership.
