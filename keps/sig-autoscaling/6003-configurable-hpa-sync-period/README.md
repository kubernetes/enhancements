...
- Newly created HPAs and spec changes (detected via `Generation` comparison)
  ...
  waiting for the rate limiter delay. This relies on the HPA `Generation`
  field being properly incremented on spec changes, which was fixed in
  [kubernetes#138228][].
...
**Interaction with stabilization windows**: A shorter `syncPeriodSeconds`
increases the number of scaling recommendations produced within a given
`stabilizationWindowSeconds` window. For example, with a 5s sync period and a
300s stabilization window the controller accumulates ~60 recommendations,
compared to ~20 with the default 15s period. Because the stabilization logic
selects the highest (scale-up) or lowest (scale-down) recommendation within
the window, more samples do not change the eventual scaling decision -- they
only make the window more granular. The semantics of `stabilizationWindowSeconds`
remain unchanged.
...
- Per-HPA observability metric exposing the effective sync period is available.
...
- [X] Other
  - Controller logs at verbosity >= 4 include the reconciliation interval
    for each HPA, allowing operators to verify the configured period is
    being respected.

Users can observe the frequency of `SuccessfulRescale` events when scaling is
active. Note that these events only fire when a scaling decision is made, not
on every reconciliation cycle. For periods where the HPA is stable (no scaling
needed), operators can increase the kube-controller-manager log verbosity to
confirm the reconciliation cadence. For Beta, we plan to introduce a per-HPA
metric (e.g. `horizontal_pod_autoscaler_reconciliation_interval_seconds`) that
exposes the observed interval between reconciliations, providing a direct signal
regardless of whether scaling occurs.
...
Yes. A per-HPA metric `horizontal_pod_autoscaler_reconciliation_interval_seconds`
exposing the actual observed interval between reconciliations would allow
operators to verify that the configured `syncPeriodSeconds` is achievable given
their metrics pipeline latency. This metric is planned for Beta graduation.
...
2026-05-07: [kubernetes#138294][] merged -- immediate enqueue on HPA creation and spec changes.
