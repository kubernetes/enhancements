...
+ 	// dynamic binding volumes by storage class
+ 	classResourcesDynamic := make(classResourceMap)
+ 	for _, provision := range podVolumes.DynamicProvisions {
+ 		if provision.Capacity == nil {
+ 			continue
+ 		}
+ 		class := *provision.PVC.Spec.StorageClassName
+ 		classResourcesDynamic[class] = &StorageResource {
+ 			Requested: 0,
+ 			Capacity:  0,
+ 		}
+ 		requestedQty := provision.PVC.Spec.Resources.Requests[v1.ResourceName(v1.ResourceStorage)]
+ 		classResourcesDynamic[class].Requested += requestedQty.Value()
+ 		classResourcesDynamic[class].Capacity += provision.Capacity.Capacity.Value()
+ 	}
...
And the `Score` function returns the sum of the static and dynamic scores multiplied by each weight. The weights are determined by the ratio of static and dynamic capacities.
...
+ 	staticScore  := pl.scorer(classResources)
+ 	dynamicScore := pl.scorer(classResourcesDynamic)
- 	return pl.scorer(classResources), nil
+ 	return (staticWeight) * staticScore + (1-staticWeight) * dynamicScore, nil;
}
```
