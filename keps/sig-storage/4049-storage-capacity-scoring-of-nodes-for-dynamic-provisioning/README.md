...
- Prefer a node with the least capacity.
- Prefer a node with the maximum capacity.

Considering the common scenario of local storage, we want to leave room for volume expansion after node allocation. The default setting is to prefer a node with the maximum capacity.
...
- (a) Prefer a node with the least capacity.
- (b) Prefer a node with the maximum capacity.
...
- When only dynamic provisioning is available:
  ...
  - When `VolumeBindingArgs` is set to "Prefer a node with the maximum capacity", is the Pod placed on the node with the largest available space?
  - When `VolumeBindingArgs` is set to "Prefer a node with the least capacity", is the Pod placed on the node that meets the requested size but has the smallest available space?
