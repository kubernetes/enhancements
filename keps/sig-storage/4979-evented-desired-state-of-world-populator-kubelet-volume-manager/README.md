...
This KEP proposes optimizing the loop iteration period (currently fixed at 100ms) in the Desired State of the World Populator (DSWP) in kubelet volume manager. The enhancement involves dynamically increasing the sleep period when no changes are detected and reacting to state pod manager channel .
...
1. Reducing the waiting period during the sync loop iteration allows pods to start more quickly.
...
The Desired State of the World Populator will listen to pod manager channel . Every changes made by pod manager will trigger the populator loop immediately .
...
Since the event is emitted by the Kubelet (Pod Manager) for the Kubelet (DSWP), the loss of the event poses a minimal risk.

## Design Details

Trigger the existing DSWP implementation using a channel provided by the Pod Manager. The Pod Manager acts as the source of truth for DSWP, and its channel listens for all changes made by it.
...
1. [ ] Verify the desired state of the world cache is updated correctly when the Pod manager events are received.
...
If reenabled, kubelet will again start updating the DSWP sleep period based on pod manager events. Everytime this feature is enabled or disabled, the kubelet will need to be restarted.
...
This feature relies on a channel provided by the Pod Manager to dynamically adjust the DSWP sleep period. So no external component(CRI for example) involved at this stage . 
...
The DSWP runs immediately or at least <= 100ms after the desired(pod manager) state of the pod has been changed.
...
- [ ] Metrics
  ...
  - Metric description:  exposing the delay period between the event emitted by pod manager and the exact time of DSWP has been executed.
  - Components exposing the metric: kubelet
...
The feature does not depend directly on the API server / etcd but on the pod manager(kubelet) behavior.
