...
- [ ] "Implementation History" section is up-to-date for milestone <!-- TODO: copy this from template and fill in -->
...
- DeletionPolicy is now a required field. The user must explicitly specify it, leaving no room for confusion.
...
4. COSI controller observes BucketClaim
   ...
   4. If BucketClaim reference set by admin on Bucket doesn't match, error without retry
...
BucketAccess {
  ...
  Spec BucketAccessSpec {
    ...
    // - ServiceAccount - This field is required. The driver will configure the system so that Pods
    //   using the ServiceAccount authenticate to the OSP backend automatically.
    // +optional
...
The returned `bucketID` should be a unique identifier for the OSP bucket known to the driver.
This value will be used by COSI to make all subsequent calls related to this bucket, so the driver must be able to correlate `bucketID` to the OSP backend bucket.
It is easiest for drivers to use the request `name` field as both `bucketID` and as the OSP backend bucket identifier, but this is not strictly required.
...
The returned `bucketID` should be a unique identifier for the OSP bucket known to the driver.
This value will be used by COSI to make all subsequent calls related to this bucket, so the driver must be able to correlate `bucketID` to the OSP backend bucket.
It is easiest for drivers to use the request `existingBucketID` field the `bucketID`, but this is not strictly required.
...
DriverGrantBucketAccessRequest{
  ...
  "authenticationType": "<[Key|ServiceAccount]>", // copied from BucketAccess.status.authenticationType
  "serviceAccountName": "<saName>", // copied from BucketAccess.spec.serviceAccountName
...
DriverGrantBucketAccessResponse {
  ...
  "credentials": { // all bucket info and credential
...
When `authenticationType` is not `ServiceAccount`, the COSI sidecar will always set `serviceAccountName` to an empty string in the gRPC call.
