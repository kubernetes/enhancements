...
Driver developers, administrators, and users should assume that read/write access also implies equivalent
access to write bucket and object metadata freely except for permissions that would allow users to
changes backend behaviors or otherwise access behaviors that should be restricted from them.
For behavior-altering metadata, drivers should implement controls via Bucket(Access)Class parameters.
Controls should allow administrators to specify default values for behavior-altering metadata.
Additionally, controls should allow administrators to allow users to read/write behavior-altering metadata.
...
If the backend bucket doesn't support all of the input protocols, error `InvalidArgument` must be returned.

The input `parameters` are optional and passed the same was as for DriverCreateBucket.
If the backend bucket isn't compatible with the input protocols, error `InvalidArgument` must be returned.
...
This ambiguity poses an issue for COSI.
It would be near-impossible to propose a portable API for all possible distinctions.
This is especially true considering the possibility that other now-unknown protocols could be added in the future.

COSI must establish a shared understanding that benefits users, administrators, and driver developers.

Based on user and driver developer feedback, the core of this control applies to data read/write permissions.
COSI also proposes to take the stance that users can assume that write permissions should allow them
to add bucket and object metadata freely except for metadata that changes backend behaviors.
