...
1. `bucketNameActual`: the name of the bucket in the object store.
1. `bucketIdentifier`: a unique value of set of values required by the driver to reach the bucket, optionally encoded by the driver into a serialized data format  Base64 encoded by sidecar.
