...
- MAYBE eventually fix all remaining invalid IPs/CIDRs in existing
  ...
  resolution. The argument against is that it's tricky and could
  possibly break some users in weird ways. The argument for it is that
  if we _don't_ do it, then theoretically all k8s users have to
  continue dealing correctly with legacy IP/CIDR values literally
  forever (but also it's impossible to have e2e tests of your legacy
...
`serviceCIDR.spec.cidrs[]` already has stricter semantics than we
propose here (it requires canonical form for IPv6 values), so while
its validation function will be updated to make better use of shared
code, its semantics will not actually change.

`service.spec.loadBalancerSourceRanges[]` has slightly anomalous syntax
for historical reasons (allowing whitespace around values). The
...
In the case of the three fields above which are not arrays themselves,
and not contained within arrays, this is simple: if `pod.spec.hostIP`,
`pod.spec.podIP`, or `service.spec.clusterIP` contains an invalid
value, and an update the the Pod/Service does not change the value of
that field, then the invalid value should be allowed to remain.
...
of array-valued fields (`node.spec.podCIDRs[]`,
`pod.spec.dnsConfig.nameservers[]`, `pod.spec.hostIPs[]`,
`service.spec.clusterIPs[]`, `service.spec.externalIPs[]`,
`service.spec.loadBalancerSourceRanges[]`, and
`serviceCIDR.spec.cidrs[]`) and fields whose immediate parent (only)
is an array (`pod.spec.hostAliases[].ip`, `pod.spec.podIPs[].ip`,
...
For the case of NetworkPolicy, Endpoints, and EndpointSlice,
validation is more complicated, because there are multiple levels of
arrays above the IP/CIDR values
(`networkpolicy.spec.egress[].to[].ipBlock.cidr`,
`networkpolicy.spec.egress[].to[].ipBlock.except[]`,
`networkpolicy.spec.ingress[].from[].ipBlock.cidr`,
`networkpolicy.spec.ingress[].from[].ipBlock.except[]`,
...
Assuming all of the UNRESOLVED sections are ignored, the updated
validation (not including apiserver warnings or a feature gate) is
already implemented, in [kubernetes #122550]; the KEP is being written
retroactively to make sure we have agreement on that plan (or not).
...
This means that we will lose testing for out-of-tree service proxy
...
N/A; No interactions between kubernetes components should be affected
by the change, so skew doesn't matter.
...
In the worst case, if the cluster's CNI plugin returned invalid pod
IPs, then all new pod creation would fail.
