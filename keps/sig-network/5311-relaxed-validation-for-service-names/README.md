...
as other of their resources.
Additionally, it simplifies the Kubernetes code base slightly, by removing the `NameIsDNS1035Label` validation,
which is only used by Service names.

### Goals

- Allow Service names to be created using RFC-1123 validation
...
When enabled, the feature gate will use the [NameIsDNSSubdomain](https://github.com/kubernetes/kubernetes/blob/3196c9946355c1d20086f66c22e9e5364fb0a56f/staging/src/k8s.io/apimachinery/pkg/api/validation/generic.go#L37-L42) validation for new Services.
...
- Creation and update of Services will use new RFC-1123 relaxed validation for `.metadata.name`
- Creation and update of of Ingress will use the new RFC-1123 validation for `.spec.[]rules.http.[]paths.backend.service.name`
