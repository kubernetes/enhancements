...
By losening the validation of Services, this allows users to name their Services with the same conventions
as other of their resources, ie: names can now start with a digit.
Additionally, it simplifies the Kubernetes code base slightly, by removing the `apimachineryvalidation.NameIsDNS1035Label` validation,
which is only used by Service names.
...
3. Downstream applications may perform validation on fields relating to Service names (ie: [ingress-ngnix](https://github.com/kubernetes/ingress-nginx/blob/d3ab5efd54f38f2b7c961024553b0ad060e2e916/internal/ingress/annotations/parser/validators.go#L190-L197) and will also need updating.

## Design Details

Introduce a new feature gate named RelaxedServiceNameValidation is disabled by default.
...
Tests of validation will be updated.
...
1. With the feature gate enabled, test that a service can be created with a name that begins with a digit
...
An initial rollout cannot fail and won't impact already-created
Services, because at the time of the initial rollout, there cannot
already be Services that match the new validation.

A rollback will work as any Service that confirms to the new validation will continue to be validated as such.
