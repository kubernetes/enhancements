...
To alleviate this problem, we will determine whether the value of `hostnameOverride` is greater than 63 bytes during the creation process and refuse to create resources that exceed 63 bytes.
...
The `hostnameOverride` field has a length limitation of 63 characters and must adhere to the DNS label standard defined in [RFC 1123](https://datatracker.ietf.org/doc/html/rfc1123).
