...
The Kubelet API on the authenticated(10250) port currently uses a very coarse
scheme for authorizing requests. For example, reading `/healthz` and calling
`/exec/…` (i.e. execute arbitrary code) require the same `proxy` subresource.
So, if an application needs to say `list` `pods` on the node, as many monitoring
and logging agents do, they must be granted the `proxy` subresource in RBAC.
Doing so grants many other powerful permissions to these agents which could be
exploited by an attacker to escalate privilege.
...
- Remove the existing subresources.
...
Add new subresources for `healthz` and `pods` endpoint while supporting the old coarse grained
proxy subresources as shown in the table below.
...
This way users who were previously using the `proxy` subresource to grant access to the
...
kubelet API to list pods on the node without having to grant the agent the `proxy` subresource and use
least privilege to grant the exact permissions the agent needs like `pods` or 
`healthz`.
...
We picked `/healthz` and `/pods` endpoints as they are read-only.
...
requests will increase. However, `kubelet` has a SubjectAccessReview response
cache, so subsequent requests should result in cache hits.
...
kubelet creates a SubjectAccessReview request to `kube-apiserver`. The
SubjectAccessReviews are currently populated with:
...
The subresource is determined by the path of the request. `kube-apiserver` upon
receiving this request will check if the user in the SubjectAccessReview is authorized
to access the relevant resource or subresource. For example, if a cluster is using RBAC
...
We will add a new feature-gate `KubeletFineGrainedAuthz` that will be defaulted to
`false` in `alpha`. When this feature-gate is set to `true` kubelet will first send
a SubjectAccessReview specifically for the `healthz` or `pods` endpoint, based on the path of the
request made to the kubelet.
If that request fails kubelet will retry with the coarse-grained verb (`proxy`).

When `kube-apiserver` communicates with the kubelet
...
- The following unit tests will be added to `nodeAuthorizerAttributesGetter`
  - When feature gate is disabled for `/healthz` and `/pods/` only 1 
  attribute with `proxy` verb should be returned
  - When feature gate is enabled for `/healthz` and `/pods/` 2 attributes should
  be returned and the first should use verb `healthz` or `pods` and the second should use 
  the `proxy` verb.
...
No, since we still send a SubjectAccessReview with the `proxy` subresource.
