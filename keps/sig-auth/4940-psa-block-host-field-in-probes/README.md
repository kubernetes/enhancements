...
* Deprecate `.Host` field from the API by updating the types.go
  doc string and any documentation updates on the website.
...
that does exactly that. Once we add the PSA, we can also deprecate the API
as part of this KEP itself.
...
These above exact set of fields is what will get deprecated as well.
...
- Creating test cases for each probe type (HTTPGet, TCPSocket)
- Testing each probe location (LivenessProbe, ReadinessProbe, StartupProbe, Lifecycle)
...
`.Host` field will be deprecated from the Probe and Lifecycle Handlers
