...
type PodCertificateRequestSpec struct {
  ...
  // userConfig allow pod authors to pass additional information to
  ...
  UserConfig map[string]string `json:"userConfig,omitempty" protobuf:"bytes,11,opt,name=userConfig"`
...
const (
  ...
  // InvalidUserConfig should be set on "Denied" conditions when the signer
...
type PodCertificateProjection struct {
  ...
  // User-requested configuration that is copied directly into the PodCertificateRequest.
...
gate), there can be transient errors.  If some kube-apiserver replicas have the
API and feature gates enabled, while others do not, it will be possible for a
Pod that uses a podCertificate projection to be admitted, but subsequently
receive errors when Kubelet attempts to create PodCertificateRequest objects.

These errors should naturally resolve themselves with retries once all
kube-apiserver replicas have the feature enabled.

There is an analogous situation during feature disablement.  In this case, the
affected pod will not recover, because it is attempting to use a feature that
kube-apiserver no longer supports.  The solution is for the user to delete their
pods that are attempting to use podCertificate projections.
...
* `podcertificate_time_to_expiration` A gauge vector, faceted on namespace, pod
  name, and volume name, reporting the seconds until expiration for all fetched
  and mounted certificates.  Note that the gauge value could go negative, if a
  certificate has not yet been refreshed.
* `podcertificate_time_to_refresh`: A gauge vector, faceted on namespace, pod
  name, and volume name, reporting the seconds until refresh for all fetched and
  mounted certificates.  Note that the gauge value could go negative, if a
  certificate has not yet been refreshed.
* `podcertificate_completed_requests_count`: A counter vector, faceted on namespace,
  pod name, volume name, and "issued|denied|failed" reporting the historical
  counts of completed PodCertificateRequests for a given pod.
* `podcertificate_num_pending_requests`: A gauge vector, faceted on namespace,
  pod name, and volume name, reporting the current number of pending
...
* `podcertificate_processed_latencies`: A histogram vector, faceted on signer name,
  key type, and "issued|denied|failed"
  reporting the time between PodCertificateRequest creation and terminal state.
* `podcertificate_processed_count`: A counter vector, faceted on signer name,
  key type, and "issued|denied|failed" reporting
  the time between PodCertificateRequest creation and terminal state.
* `podcertificate_num_pending_requests`: A gauge vector, faceted on signer name
  and key type reporting the current count of pending PodCertificateRequests.
...
Given a properly functioning signer, Kubelet's SLOs could be:
* No certificates are reported as having gone more than 10 minutes beyond their
  indicated refresh time.
...
- [x] Metrics
  - Metric name: `kubelet/podcertificate_time_to_refresh`
    - The minimum value over all streams should be greater than -600.  If it is
      lower, it indicates that at least one `kubelet` instance has allowed a
      certificate to go 10 minutes past its indicated refresh time without being
      refreshed.
