...
- [Proposal](#proposal)
  ...
  - [New logging formats](#new-logging-formats)
    ...
    - [Development format](#development-format)
...
- [Design Details](#design-details)
  ...
  - [Graduation Criteria](#graduation-criteria)
    ...
    - [Beta](#beta)
    - [GA](#ga)
    - [Deprecating old logging format](#deprecating-old-logging-format)
...
- [Alternatives](#alternatives)
    ...
    - [Avoid double writing of logs using klog and logr APIs](#avoid-double-writing-of-logs-using-klog-and-logr-apis)
...
Access to context variables should be done though predefined functions. For that we propose to a new module to kubernetes component base called `kontext`. This new module will include all utility functions for storing and retrieving logger and metadata needed by it. 

Throughout call stack new metadata can be added to context that can be later used by logger. For example reconciliation loop can reference the reconcilided object so that all logs will be referencing it. All metadata access should be done through functions in `kontext` module. Adding reference to kubernetes object (pod, service, node) should be really straightforward.
...
func updatePodStatus(ctx context.Context, pod *v1.Pod, newStatus bool) {
  logger := kontext.GetLogger(ctx)
  logger.V(6).Info("Updating Pod status")
  [...]
  logger.V(4).Info("Pod status updated", "status", newStatus)
}
```

### New logging formats

Introduction of new logging API should be paired with new output formats that can properly utilize them. In order to simplify log ingestion, but still maintain similar developer experience we would like to propose two formats. First one for production use and second one for component development.

#### Production format

We would like to propose a JSON as a default production format in Kubernetes. Some pros of using JSON:
* Broadly adopted by logging libraries with very efficient implementations (zap, zerolog).
* Out of the box support by many logging backens (Elasticsearch, Stackdriver, BigQuery Splunk)
...
```json
{"ts": "00:15:15.525108", "caller": "controller_utils.go:116", "msg": "Updating ready status", "core/node": {"name": "node-1"}, "core/pod": {"name": "nginx-1", "namespace": "default"}, "controller": {"status": false}}
```

#### Development format

Dedicated for eyeballing by kubernetes developers. This format is expected to change with time as there is no need to maintain any guarantee for its compatibility. Currently proposed format is based on [glogr](https://github.com/go-logr/glogr). 

Example:

```
I1025 00:15:15.525108       1 controller_utils.go:116] controller “msg”=”Updating ready status” "core/node": {"name": "node-1"} "core/pod": {"name": "nginx-1", "namespace": "default"} "controller": {"status": false}
```

### Logging configuration

 
A new flag will be introduced to all kubernetes components allowing users to select their preferred logging format and maintain backward compatibility.
Proposed flag `--logging-format`, with 3 values:
* `legacy` for old logging format
* `json` for json production format
* `dev` for development format

Setting this flag will select particular logger implementation from registry. Any additional configuration needed by loggers should be provided by flags specific to its implementation. Default loggers should provide a basic customization, but it will not be encouraged to avoid feature bloat. Adding new loggers, changing logger behavior or extending its functionality should be done outside of Kubernetes by implementing logr API and registering it in code.

We acknowledge that extension via code is not the best, but should be sufficient for now. 

All logs from the `json` and `dev` loggers should go to standard output.  Out of the existing klog command line arguments `json` and `dev` logger should support ‘-v’ flag.  
...
TODO(serathius): Propose how definition format should look like, probably yaml similar to https://github.com/elastic/ecs/tree/master/schemas
...
This stage needs to be performed in a single release of Kubernetes. When finished for every **existing** log entry produced in legacy output format there should be a corresponding log entry in json/dev format containing similar information possibly with some additional context information attached to it.
...
After this stage format and content of json/dev formatted logs will be still in pre-alpha state and most likely will change after manual review. 
...
#### Preparation
* Develop and merge to Kubernetes master 3 implementations/configurations of logr API which will be needed in the latter steps of the migration:
    * `no-op` implementation which does not log anything,
    * `json` log format implementation,
    * `dev` log format implementation.
* Implement `k8slog` package which during its initialization should read `--logging-format` flag and initialize two global variables providing access to:
    * logr implementation to be used to migrate existing logs (json/dev loggers for json/dev log format and no-op logger for legacy format)
    * logr implementation to be used by new logs (json/dev loggers for json/dev log format and klogr logger for legacy format)
...
1. If `json` or `dev` log format is selected klog v2 library should be initialized with json or dev logr implementation so that all logs produced by Kubernetes or its dependencies using klog API would be redirected to logr.
...
1. New kontext package should be implemented which would provide functionalities related to Context handling in Kubernetes:
...
1. Starting from the identified entry points Context containing a logger should be propagated to all places where we log anything. The propagation should be done directly by passing Context as an argument to called functions/methods or indirectly by associating a Context with existing objects passed between functions/methods. In all places where context has been added references to module logger should be replaced with `kontext.getLogger(context)` calls. 
...
TODO(serathius):No major impact when switching from legacy to dev format
...
We are expecting improving performance of logger library due ability to change logging library to more efficient, then current `klog`. 


TODO(serathius): provide performance comparison and estimate log size change

As part of this proposal we expect logging to be more dependent on usage of context, which can create overhead when adding to and passing it through call stack. We accept this overhead as cost of improving observability in K8s. There is a proposal that would benefit [Leveraging Distributed Tracing to Understand Kubernetes Object Lifecycles](https://github.com/kubernetes/enhancements/pull/650#issuecomment-552577896).
...
| - |-  |
`klog.V(level).Info*(format/args[0], args)` | `<module>.Logger.V(level+2).Info(format/args[0], “args”, args)`
`klog.Info*(format/args[0], args)` | `<module>.Logger.V(2).Info(format/args[0], “args”, args)`
`klog.Warning*(format/args[0], args)` | `<module>.Logger.V(1).Info(format/args[0], “args”, args)`
`klog.Error*(format/args[0], args)` | `<module>.Logger.Error(error(format/args[0]), format/args[0], “args”, args)` or after manual verification that this is not a proper error log: `<module>.Logger.Info(format/args[0], “error”, error(format/args[0]), “args”, args)`
`klog.Fatal*(format/args[0], args)` | `<module>.Logger.Error(error(format/args[0]), format/args[0], “args”, args, “stacks”, k8slog.AllStacks()) os.Exit(255)`
`klog.Exit*(format/args[0], args)` | `<module>.Logger.Error(error(format/args[0]), format/args[0], “args”, args) os.Exit(1)`
...
* `klog` is deprecated and should not be used for new logs
...
#### Deprecating old logging format

* After 2 k8s versions old logging format removed 

### Upgrade / Downgrade Strategy

Double writing during migration will ensure identical logs without any configuration changes. Switching to new format will require passing a flag to each component or waiting for full removal of klog.
...
#### Avoid double writing of logs using klog and logr APIs

Instead of leaving the existing references to klog (or actually a klog wrapper) for an extended period of time until the legacy output format is completely removed from the Kubernetes and having to do a double writing to both klog and logr APIs for all existing logs we could consider two alternative approaches:
...
This however would make accidental breaking of the legacy output format compatibility much easier and would limit the flexibility when it comes to changes which can be introduced in the `json`/`dev` output format logs as it would be for instance harder to move API calls which produce those.
...
* How can this feature be enabled / disabled in a live cluster? **No, provided by flag**
* Can the feature be disabled once it has been enabled (i.e., can we roll back the enablement)? **Yes**
* Will enabling / disabling the feature require downtime for the control plane? **Yes**
* Will enabling / disabling the feature require downtime or reprovisioning of a node? **Yes**
* What happens if a cluster with this feature enabled is rolled back? What happens if it is subsequently upgraded again? **Difference in log format**
* Are there tests for this? TBD
...
Will enabling / using the feature result in increasing time taken by any operations covered by existing SLIs/SLOs (e.g. by adding additional work, introducing new steps in between, etc.)? 
Will enabling / using the feature result in non-negligible increase of resource usage (CPU, RAM, disk IO, ...) in any components? Things to keep in mind include: additional in-memory state, additional non-trivial computations, excessive access to disks (including increased log volume), significant amount of data sent and/or received over network, etc. Think through this in both small and large cases, again with respect to the supported limits. **Yes, increased log volume**
...
* Monitoring requirements
    * How can an operator determine if the feature is in use by workloads? Flag, log format
