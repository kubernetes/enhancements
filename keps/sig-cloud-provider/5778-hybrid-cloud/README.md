...
* CCM is not responsible for configuring networking between clouds or on-premises environments.
* Cross-cloud or cross-on-prem load balancing is not supported. Load balancers must only target nodes in the same cloud where the load balancer is created.

## Proposal

Today, many cloud providers already implement their own internal logic to decide whether a node belongs to their cloud or not.
Those implementations are vary, have hidden knowledge how to identify nodes from different clouds.

Move node ownership and responsibility into the Cloud Provider framework so it becomes explicit, standardized, and predictable.
...
* CCM Y must:
  ...
  * manage Y-specific networking and routes
...
* add a new method to the Cloud Provider interface to return supported platform identifiers
* add a new command-line flag to the CCM to override (limited) set of supported platform identifiers
* define the annotation to identify the platform when the ProviderID cannot be defined from the virtual machine
...
Route controller:
* Native pod routing can be only supported in one environment. In hybrid envirolment CNI takes care of it.
* Should be disabled in hybrid environment.
