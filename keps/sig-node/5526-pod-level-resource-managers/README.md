...
2.  **Implement Hybrid Allocation Logic:** The CPU and Memory managers will
    ...
    Manager's scope:
    *   **Pod Scope:** A single NUMA-aligned resource pool will be aligned for
        the entire pod using `pod.spec.resources` and then partitioned.
        Exclusive slices will be allocated to `Guaranteed` containers, while a
        exclusive shared pool will be allocated to all other containers. This
        partitioning enables a single pod to host a mix of containers with and
        without specific resource requests.
...
remaining resources within the pod's exclusive shared pool.
...
high-performance NIC). For optimal performance, the sidecar requires exclusive,
`Guaranteed` resources for NUMA alignment. My main workload also has resource
...
critical step for device co-location), while the main workload, with its
distinct requirements, can co-exist in the same pod and utilize other available
resources.
...
check on each container to determine which are eligible for exclusive resource
allocation. To aid debugging, the results of this check will be surfaced to via
log message.
...
3.  **Handling Init Containers:** This KEP explicitly reuses the existing,
    well-established logic for handling init container resources and applies it
    within the new pod-level resource pool. The allocation is pre-calculated
    based on the predictable, sequential lifecycle of init containers.
...
5.  **Handling Init Containers:** This KEP explicitly reuses the existing,
    well-established logic for handling init container resources and applies it
    within the new pod-level resource pool. The allocation is pre-calculated
    based on the predictable, sequential lifecycle of init containers.
    ...
    -   **Standard Init Containers:** Following the current logic, if a standard
        init container is individually `Guaranteed`, it is allocated an
        exclusive slice of memory from the pod's total resource set. At
        allocation time, this memory is also added to a per-pod `reusable` pool,
        making it available for subsequent app containers (whether for exclusive
        allocations or within the `podSharedPool`).
    -   **Restartable Init Containers (Sidecars):** Consistent with the existing
        sidecar behavior, if a restartable init container is individually
        `Guaranteed`, it is allocated an exclusive slice of memory that is
        reserved for the entire pod lifecycle and is *not* added to the reusable
        pool.
...
*   **Rollout Process (Enabling the Feature)**
    ...
    1.  **Step 1: Canary Rollout:** Perform the upgrade on a small, isolated
        group of "canary" nodes first. This involves enabling the
        `PodLevelResourceManagers` feature gate in the Kubelet configuration and
        restarting the Kubelet on those nodes. This limits the blast radius of
...
*   **Rollback Process (Disabling the Feature)**
    ...
    1.  **Step 1: Kubelet Configuration Change:** On the affected nodes (either
        ...
        (`--feature-gates=PodLevelResourceManagers=false`).
...
-   `pod_resource_manager_exclusive_allocation_pods`
...
-   [ ] New Metrics
    ...
    -   Metric name: `pod_resource_manager_exclusive_allocation_pods`
...
intentionally deferring more granular observability metrics to a future
iteration if they are required.
...
-   [ ] Missing Metrics
    ...
    -   Metric name: `pod_cpu_manager_exclusive_containers`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_cpu_manager_exclusive_shared_pool_containers`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_memory_manager_partitioned_pods`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_memory_manager_exclusive_containers`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_memory_manager_exclusive_shared_pool_containers`
        -   Aggregation method: `gauge`
