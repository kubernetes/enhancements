...
performance-sensitive workloads. All functionality will be controlled by a new
...
functionality described in this KEP. When this gate is active, the Topology,
CPU, and Memory managers will be updated to recognize and act upon
`pod.spec.resources`.

The core of this proposal is to enable two flexible resource management models,
both of which support `Guaranteed` pods that contain a mix of `Guaranteed` and
...
This approach provides significant flexibility by allowing users to co-locate
critical, performance-sensitive containers with less critical sidecars or helper
containers, all within a single, NUMA-aligned pod.
...
Yes, but the impact is expected to be minimal and comparable to the existing
latency introduced by the Topology Manager when making alignment decisions for
container-level resources. The new logic for pod-level resources follows a
similar path of hint generation and merging, so the additional overhead should
not be significant. This is a one-time cost at pod creation, and the performance
benefits for the running workload are expected to outweigh this initial latency.
The `topology_manager_admission_duration_seconds` metric will be used to monitor
this.
