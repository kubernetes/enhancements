...
2.  **Implement Hybrid Allocation Logic:** The CPU and Memory managers will
    ...
    Manager's scope:
    *   **Pod Scope:** A single NUMA-aligned resource pool will be managed for
        the entire pod. This pool will then be partitioned, with exclusive
        slices allocated to `Guaranteed` containers and a shared pool allocated
        to all other containers.
    *   **Container Scope:** The existing per-container allocation logic will be
        preserved. This allows containers with their own `Guaranteed` requests
        to receive exclusive NUMA-aligned resources while others in the same pod
        use the node's shared pool.

This updated approach provides significant flexibility by decoupling the
pod-wide alignment decision from the per-container allocation, allowing users to
co-locate critical, performance-sensitive containers with less critical ones.
...
provides the flexibility to NUMA-align individual containers within a pod
independently. This is essential for workloads with complex, per-container
hardware affinity requirements, such as a pod with a main workload and an
infrastructure sidecar that needs to be placed on a specific NUMA node to access
a hardware device. While the main workload can be placed on a different NUMA
node to avoid resource contention, the sidecar can be aligned to its required
NUMA node.
...
-   When the `container` scope is active, the resource managers will ignore
    ...
    per-container requests. However, `pod.spec.resources` will still be used to
    determine the pod's QoS class.
...
pod's QoS class. However, the resource managers will still perform a QoS-like
check on each container to determine which containers are eligible for exclusive
resource allocation. To enable debugging, the results of this check will be
surfaced to the user via a log message.
...
New behavior <br> Full    | Pod: 5 CPU <br> Container 1: 3 CPU <br> Container 2: 1 CPU <br> Container 3: 1 CPU | {NUMA node 0}   | Container 1: {1, 2, 3} <br> Container 2: {4} <br> Container 3: {5}
New behavior <br> Partial | Pod: 5 CPU <br> Container 1: 3 CPU <br> Container 2 <br> Container 3               | {NUMA node 0}   | Container 1: {1, 2, 3} <br> Container 2: {4, 5} <br> Container 3: {4, 5}
New behavior <br> None    | Pod: 5 CPU <br> Container 1 <br> Container 2 <br> Container 3                      | {NUMA node 0}   | Container 1: {1,2,3,4,5} <br> Container 2: {1,2,3,4,5} <br> Container 3: {1,2,3,4,5}
...
-   Both `container-1` and `container-2` will be assigned the same 2 CPU set.
...
purpose. This allows for independent NUMA alignment for each container within
the pod, enabling a mix where some containers receive exclusive resources while
others run in the node's shared pool.
...
allocation. To enable debugging, the results of this check will be surfaced to
...
New behavior <br> Full    | Pod: 5 CPU <br> Container 1: 3 CPU <br> Container 2: 1 CPU <br> Container 3: 1 CPU | {NUMA node 0}, {NUMA node 0}, {NUMA node 0}                | Container 1: {1, 2, 3} <br> Container 2: {4} <br> Container 3: {5}
New behavior <br> Partial | Pod: 5 CPU <br> Container 1: 3 CPU <br> Container 2 <br> Container 3               | {NUMA node 0}, No NUMA preference, No NUMA preference      | Container 1: {1,2,3} <br> Container 2: no exclusive allocation <br> Container 3: no exclusive allocation
New behavior <br> None    | Pod: 5 CPU <br> Container 1 <br> Container 2 <br> Container 3                      | No NUMA preference, No NUMA preference, No NUMA preference | Container 1: no exclusive allocation <br> Container 2: no exclusive allocation <br> Container 3: no exclusive allocation
...
`container-2` gets an exclusive allocation, while `container-1` runs in the
...
-   `container-2` will be assigned its own exclusive 1 CPU set.
...
The following algorithm describes the process for the CPU Manager:

1.  **[New Logic] One-Time Pod-Level Calculation:** When the `Allocate` function
    is called for the first container of a pod, the manager performs a one-time
    calculation for the entire pod. It determines the total pod resource budget
    from `pod.spec.resources` and uses the Topology Manager's hint to identify
    the single, NUMA-aligned (if a suitable alignment was found) set of CPUs
    that will be managed for the pod.

2.  **[New Logic] Calculation of Shared Pool:** The manager calculates the
    `podSharedPool` by taking the total allocated pool and subtracting the sum
    of all exclusive CPU requests from the pod's `Guaranteed` containers (both
    init and app containers).
...
3.  **Handling Init Containers:**
    ...
    -   **Standard Init Containers:** For init containers that are individually
        `Guaranteed`, an exclusive slice is carved out from the pool. Upon
        completion, these CPUs are added to a per-pod `reusable` pool, making
        them available to subsequent app containers within the pool.
    -   **Restartable Init Containers (Sidecars):** For restartable init
        containers that are individually `Guaranteed`, an exclusive slice is
        carved out and reserved for the entire pod lifecycle. These CPUs are
        *not* added to the reusable pool.
...
The following algorithm describes the process for the Memory Manager:

1.  **[New Logic] One-Time Pod-Level Calculation:** When the `Allocate` function
    is called for the *first* container of a pod, the manager performs a
    one-time calculation for the entire pod. It determines the total pod
    resource budget from `pod.spec.resources`.

2.  **Hint Selection:** The manager retrieves the NUMA affinity hint from the
    Topology Manager. It then performs a series of checks to select the
    `bestHint`, which may involve getting a default hint or extending the
    initial hint if it is not sufficient to satisfy the pod's request.

3.  **NUMA Violation Check:** The manager verifies that the `bestHint` does not
    violate the rule against mixing single- and multi-NUMA allocations on the
    same node.

4.  **[New Logic] Calculation of Shared Pool:** The manager calculates the
    `podSharedPool` by taking the total pod resource budget and subtracting the
    sum of all exclusive memory requests from the pod's `Guaranteed` containers
    (both init and app containers).

5.  **Handling Init Containers:**

    -   **Standard Init Containers:** For init containers that are individually
        `Guaranteed`, an exclusive slice of memory is carved out from the
        managed set. Upon completion, this memory is added to a per-pod
        `reusable` pool, making it available to subsequent app containers within
        the pod's shared pool.
    -   **Restartable Init Containers (Sidecars):** For restartable init
        containers that are individually `Guaranteed`, an exclusive slice of
        memory is carved out and reserved for the entire pod lifecycle. This
        memory is *not* added to the reusable pool.

6.  **[New Logic] Pre-computation and State Update:** The manager pre-computes
    and writes the specific assignment for *every* container to the state file.
    `Guaranteed` containers are assigned their exclusive slices, and all other
    containers are assigned the `podSharedPool`.

7.  **Container-Level Allocation:** For the current container (and all
    subsequent containers in this pod), the manager will simply read the
    pre-calculated assignment from the state and apply it to the container's
    cgroup.
...
-   **`pod_topology_manager_pinning_errors_total`**: A rise in this metric
    indicates that the Topology Manager is failing to find suitable NUMA
    alignments for pods utilizing pod-level resources. This suggests a
    fundamental issue with the new hint generation or merging logic.
-   **`pod_cpu_manager_pinning_errors_total`**: An increase here points to
    failures in the CPU Manager's new partitioning or allocation logic for
    pod-level resources. This could manifest as pods failing to acquire their
    requested CPU sets.
-   **`pod_memory_manager_pinning_errors_total`**: Similarly, a spike in this
    metric signals problems with the Memory Manager's new partitioning or
    allocation for pod-level memory, potentially leading to memory allocation
    failures.
...
-   `pod_resource_manager_pods_total`
-   `pod_topology_manager_pinning_requests_total`
-   `pod_cpu_manager_pinning_requests_total`
-   `pod_memory_manager_pinning_requests_total`

By aggregating these metrics across the cluster (e.g., with a `sum()` query in a
monitoring system), an operator can get a clear picture of the overall adoption
and usage of the feature without needing to inspect individual nodes.
...
-   [ ] Metrics
    -   Operators can monitor the new Kubelet metrics:
        `pod_resource_manager_pods_total`,
        `pod_topology_manager_admission_requests_total`,
        `pod_cpu_manager_pinning_requests_total`, and
        `pod_memory_manager_pinning_requests_total` to see if pod-level pinning
        requests are being made. The corresponding `_errors_total` metrics can
...
-   [ ] New Metrics
    ...
    -   Metric name: `pod_resource_manager_pods_total`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_topology_manager_admission_requests_total`
        -   Aggregation method: `sum()`
    -   Metric name: `pod_topology_manager_admission_errors_total`
        -   Aggregation method: `sum()`
    -   Metric name: `pod_cpu_manager_pinning_requests_total`
        -   Aggregation method: `sum()`
    -   Metric name: `pod_cpu_manager_pinning_errors_total`
        -   Aggregation method: `sum()`
    -   Metric name: `pod_memory_manager_pinning_requests_total`
        -   Aggregation method: `sum()`
    -   Metric name: `pod_memory_manager_pinning_errors_total`
        -   Aggregation method: `sum()`
...
For the `pod` scope, it would also be beneficial to have metrics that track the
number of pods with partitioned resources and the number of containers with
exclusive slices versus those in the shared pool. To keep the initial
implementation focused on essential health and adoption signals, we are
intentionally deferring more granular observability metrics to a future release
if they are required.

-   [ ] Missing Metrics

    -   Metric name: `pod_cpu_manager_partitioned_pods`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_cpu_manager_exclusive_containers`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_cpu_manager_shared_pool_containers`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_memory_manager_partitioned_pods`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_memory_manager_exclusive_containers`
        -   Aggregation method: `gauge`
    -   Metric name: `pod_memory_manager_shared_pool_containers`
        -   Aggregation method: `gauge`
