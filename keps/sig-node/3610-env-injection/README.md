...
For namespace-wide, it would be a problem to make an env to all namespaces. There should be a patched annotation after new namespace is created. 
...
A global setting like proxy or env that used by default in some clients.

- This would also be very useful when running a service mesh like Linkerd within the cluster, as you have to set the http_proxy environment for everything that should use it.


#### Story 2

From <https://github.com/kubernetes/kubernetes/issues/48180#issuecomment-331941326>
> Automatically inject the kubernetes namespace into an environment variable for me. With dozens of different kube templates people sometimes
> forget to expose it manually (<https://kubernetes.io/docs/tasks/inject-data-application/environment-variable-expose-pod-information/>)
> Have compute nodes have special knowledge about themselves that can be injected into environment variables. We have an odd/even compute node
> concept for upgrading openshift and this information may come in handy
> Be able to specify system-wide environment variables. We have multiple openshift instances (different environments and different data centers).
> It would be great if that data could be made automatically available without having to deal with custom env sections in configs.
...
- `plugin/pkg/admission/globalenv/`: `2022-10-10` - `0%`
...
- Feature implemented as an admission contoroller that is not enabled by default
...
-->
After enabling it, new pods creation will be injected with envs.
After rolling back, new pods reation will not be injected with envs.
...
- [x] Other
  - Describe the mechanism: Admission Controller `GlobaelEnv`
...
Reenabling is the same as enabling the feature.
...
-->
If there are some failures, skip them and just log some warnings.
...
-->
No easy way. Check the apiserver startup params.
...
-->
A little slow down pod creation as it adds a step in pod creation.
...
-->
- initial pr: https://github.com/kubernetes/kubernetes/pull/112787/
