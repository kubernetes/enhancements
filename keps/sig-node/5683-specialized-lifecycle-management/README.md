...
If there's no unexpected interuption, then garbage collection will occur when an Event is in an end state: `Succeeded`, `Failed`, and `SLAExpired`. While retaining `LifecycleEvents` in these end states can be valuable for debugging, we're proposing that solution be developed outside this proposal. For this KEP, the gargabe collection sequence will be as follows:
...
type LifecycleTransition struct {
    ...
    // NodeName identifies the Node(s) that has a Driver capable of
...
type LifecycleEventStatus struct {
    ...
    // DriverName is a copy of the driver name from the LifecycleTransition.
    // It's needed by the kubelet to match with a registered Driver.
    Driver string
...
const (
    ...
    // The LifecycleEvent was attempted and succeeded
