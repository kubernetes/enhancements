...
- This KEP does not provide a centralized engine to decide when a Node should be drained, undergo maintenance; it only provides the interface for how lifecycle intent is communicated
...
spec:
  ...
  nodeName: Node01
  driverName: server_side_kubectl_drain
```
...
The Kubelet will `PATCH` the Node’s `.status.conditions` after each callback:
```yaml
  conditions:
...
For an Event to conclude, either the Driver returns the end callback to the Kubelet or the SLA expires. If the SLA expires, Kubelet will change the Event to `SlaExpired` state and look for more claims.

The Kubelet will use a Finalizer on the `LifecycleEvent`. The finalizer must be set before the Driver attempts the start transition, after the `LifecycleEvent` is claimed. Kubelet removes the finalizer when:
- The transition times out
...
This reference implementation will hosted as a sub-project under kubernetes-sigs and will include:
...
- The repo will include a script that will orchestrate the transition from _DrainStarted_ to _MaintenanceComplete_

By implementing Server-Side Drain via the SLM API, we prove that context-specific logic can be offloaded to a driver while maintaining a standard observability interface in core Kubernetes.

### API

The `LifecycleTransition` API will have the following structure:
```Go
// LifecycleTransition defines the desired intent for a K8s resource API to undergo a
// lifecycle change. It acts as the source of truth for the requested start and
// end states, the location of the change, and how long it is allowed to take.
type LifecycleTransition struct {
    ...
    // Condition on the target K8s resource API.
    End string
...
The `LifecycleEvent` API will have the following structure:
```Go
...
type LifecycleEventSpec struct {
    // The TransitionName referes to a LifecycleTransition object.
    TransitionName string

    // DriverName specifies the unique identifier of the lifecycle driver
    // responsible for reconciling this transition.
    DriverName string

    // NodeName identifies a specific Node whose Kubelet can claim the
...
type LifecycleEventStatus struct {
    // DriverName is a copy of the driver name from the LifecycleDriver.
    // It's needed by the kubelet to match to a registered Driver.
    DriverName string
...
const (
    ...
    // The LifecycleEvent was attempted to be claimed, but failed
    LifecycleEventFailed LifecycleEventClaimStatus   = "Failed"

    // LifecycleSlaExpired indicates that the transition failed to reach
    // the End state within the duration specified in the
    // LifecycleTransition SLA.
    LifecycleSlaExpired LifecycleEventClaimStatus   = "SlaExpired"
)
```
...
reference specalized Lifecycle Driver, Kubectl-server-side Drain. These tests
will include:
- Coordinating an e2e Node drain
- Verifying the Kubelet only claims one `LifecycleEvent` at a time
- An expired sla transitions a `LifecyleEvent` to `SlaExpired` state
- A failed claim by the Kubelet transitions the `LifecyleEvent` to `Failed` state
...
- a user can not longer use proposed APIs
