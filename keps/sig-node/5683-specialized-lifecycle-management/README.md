...
Over the past several years, it has become a common pattern to use Taints, PDBs, Labels, and Annotations to coordinate lifecycle operations between controllers. While this technique is flexible and easy to use, it is brittle, has little reusability across projects, and offers limited precision. Now with new workloads entering the Kubernetes ecosystem through the introduction of Dynamic Resource Allocation (DRA), the day-2 lifecycle will get even more challenging, overwhelming these commonly used techniques.
...
This project does not replace Taints, PDBs, Labels, and Annotations. Its intent is to replace the misuse of Taints, PDBs, Labels, and Annotations to store lifecycle state and coordinate lifecycle operations.

## Motivation

There are several techniques used to manage complex lifecycle transitions today that are not ideal. These are a combination of Conditions, Labels, Taints, and Annotations [^1]. All of these solutions have in common that they are extensible and easy to use, but lack the necessary structure to be reusable across projects.
...
A primary example of this is the "Drained" state for Nodes. The definition of a "Drained" node is highly context-specific: for some, it means zero pods; for others, it allows for specific DaemonSets or high-priority system pods to remain. Without a formal interface to define and observe these lifecycle states on the Node, it is nearly impossible to build shared, reusable controllers that react consistently across different clusters.
...
- We don't plan on creating new Status field on K8s Resource APIs to track lifecycle state, for now (Conditions will work in Alpha, so we can revisit this later)

### Extension Points

This KEP is meant to be a building block for later extentions. Here are some ways we think SLM can be extended with in-tree work or ecosystem projects:

- Device Lifecycle Management
- Lifecycle Management of Future Resource APIs
- Multi-Node Lifecycle Management
- Topology Aware Maintenance
- Maintenance Scheduler
- Maintenance Orchestration API
...
- If a suitable Driver cannot be located, skip claiming the `LifecycleEvent` in this cycle and retry in a later reconcile loop.
- If a suitable Driver is registered but does not become ready within 30s during claim, change the `LifecycleEvent` to `Failed` state
- Transition from `Pending` to `Claimed` if the latest object state is still `Pending`
...
- If a driver callback (`StartLifecycleTransition(...)` or `EndLifecycleTransition(...)`) returns an error, Kubelet transitions the `LifecycleEvent` to `Failed` state.

This approach has some challenges. First, in order to deal with multiple `LifecycleEvents`, the Kubelet will claim only one `LifecycleEvent` at a time on a best-effort basis. The Kubelet will attempt to claim the first `LifecycleEvent` it sees, but this is not a strict fifo.

The intent of claiming LifecycleEvents serially is enable lifecycle ownership. The driver working on the lifecycle transition should only focus on lifecycling the resource object. Drivers should not need to worry about another driver lifecycling the object concurrently.

While Kubelet will enforce a single `Claimed` lifecycleEvent for a bindingNode, "Double claiming" can still occur. Things like a stale informer cache, reconcile retries, or Kubelet restarts in the middle of claim processing, ect.., can all lead to multiple `Claimed` events. 

It's possible for the Kubelet to come across a `LifecycleEvent` that is already claimed. This can occur when the Kubelet "forgot" it previously claimed the Event, and so it will attempt to continue work. Or another controller may have claimed the Event and the Kubelet will ignore it. The Kubelet will only attempt to claim when there is not another `Claimed` Event on the Node.
...
#### Driver Deregistration Cleanup
When a Driver deregisters from a Node and does not re-register within the cleanup delay (30s), Kubelet will clean up stale `LifecycleTransition` objects for that Driver only when they are explicitly node-scoped to that same Node (`spec.nodeName=<this node>`).

Kubelet cleanup must not target cluster-scoped transition intents like `allNodes` or `nodeSelector` transitions. If Kubelet does not have a concrete Node identity, this cleanup path is skipped.
...
### Kubelet Amnesia Bug
The long-term goal for Specialized Lifecycle Managment is to build an ecosystem of controllers that understand important lifecycle operations. A good place to begin integrating core k8s into this ecosystem is with the kubelet.

The bug [kubernetes/issues/122674](https://github.com/kubernetes/kubernetes/issues/122674), which we refer to as Kubelet Amnesia, occurs because the kubelet can be interupted during GNS. The kubelet holds the GNS "state" in memory, which gets lost if the kubelet is interupted before GNS completes. We need a well-known location to persist this lifecycle state, and this will leverage the SLM framework.

Sharing the lifecycle state on the Node makes it so kubelet can read its current state. If there an interruption, kubelet will return to the Node and lookup what it should be doing. So if kubelet was draining Pods before interruption, it will return to draining Pods on return, instead of being a normal kubelet.

The kubelet must become a reader of state only. If kubelet were to publish this state to the Node, writing the state can be interrupted resulting in the same bug. A node local lifecycle driver has the same problem. If a node is powered off before the local lifecycle driver Pod posted state to the Node, it's the same bug. Writing state has to come from outside the Node.

#### Centralized Lifecycle Controller
The design so far has described a node local Driver working with the Kubelet to coordinate lifecycle operations - a centralized controller is also viable. The design allows for a controller that meets the driver contract run on the control-plane. For the Kubelet Amnesia bug, we'll need a centralized controller.

We'll extend the Kubernetes Controller Manager to house a Gaceful Node Shutdown driver to help with Kubelet Amnesia. The GNS driver, like the node local driver, owns `Lifecycle Transitions`. For the GNS driver, these will be:

| Transition Name | Description |
|---|---|
| Draining | GNS is removing Pods |
| ShuttingDown | GNS has completed removing Pods and is powering down the Node |

*note: the centralized lifecycle controller and the node local driver are NOT competing solutions - users should use the one that fits their use case.*

#### Establishing Meaningful State in the Ecosystem
*This is easier said than done, so let's start small*

The first meaningful lifecycle states will be `Draining` and `ShuttingDown` from the Graceful Node Shutdown driver. These states will communicate lifecycle states to any observer. Over time, the goal is to expand this list to include more Transitions and qualify drivers that Kubernetes should listen to.

#### Graceful Node Shutdown Driver
To fulfill the Lifecycle Driver contract, the GNS Driver needs to publish state and manage its `LifecycleEvents` and `LifecycleTransition`. Here's how the full lifecycle will look:
- GNS Driver creates the GracefulNodeShutdown `LifecycleTransition`
- GNS Driver watches for the GracefulNodeShutdown `LifecycleEvent` and updates the status to `Claimed`
- GNS Driver updates the lifecycle Node Condition to `Draining`
- Kubelet learns the GNS Driver has put the Node in `Draining` state from the Node and `LifecycleEvent`
- Kubelet drains the Node
- GNS Driver sees all Pods are gone and transitions the Node to `ShuttingDown`
- A Provider controller sees the GNS Driver posted `ShuttingDown` on the Node and powers off the machine
...
For a `LifecycleEvent` to conclude, either the Driver returns the end callback to the Kubelet or the SLA expires. If the SLA expires, Kubelet will change the Event to `SlaExpired` state and look for more claims. If the driver does return the end callback, Kubelet will change the Event to `Succeeded` state. If any driver callback (`StartLifecycleTransition(...)` or `EndLifecycleTransition(...)`) fails, Kubelet will change the Event to `Failed` state.
...
type LifecycleEventStatus struct {
	...
	// sla is a copy of the sla deadline by which the transition must complete.
	// +optional
	Sla *metav1.Time `json:"sla,omitempty" protobuf:"bytes,3,opt,name=sla"`

  	// lifecycleDriverStatus holds driver information
	// +optional
	Driver LifecycleDriverStatus `json:"driver,omitempty" protobuf:"bytes,3,opt,name=driver"`

}

// LifecycleDriver represents important driver information and status
type LifecycleDriverStatus struct {
	// name is a copy of the driver name from the LifecycleTransition.
	// +optional
	Name string `json:"name,omitempty" protobuf:"bytes,2,opt,name=name"`

	// start is a copy of the start transition from the LifecycleTransition object.
	// +optional
	Start string `json:"start" protobuf:"bytes,2,opt,name=start"`

	// end is a copy of the end transition from the LifecycleTransition object.
	// +optional
	End string `json:"end" protobuf:"bytes,2,opt,name=end"`
}
...
### Expanding to Resource APIs
We will be ONLY focusing on lifecycling the Node API for alpha. Later, we want to expand the SLM APIs to support lifecycle use cases for things like Devices, multi-Node resources, rack-level resources, switches and routers.

The API details will evolve in beta to support these use cases. For now, here is how we are thinking of extending the API later.
```Go
	// targetRef is the resource object being lifecycled. It's where the lifecycle
