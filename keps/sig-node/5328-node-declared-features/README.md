...
6. Enable Cluster Autoscaler to take better scaling decisions based on the node declared features while scaling existing node groups with active nodes.
...
4. Fully feature-aware Cluster Autoscaler decisions for scale-from-zero scenarios. The scheduler plugin would not block scale-up from templates lacking declared features, but true scale-from-zero support remains a non-goal for this KEP.
...
3.  Validation Rules
    ...
    *   Every registered feature, must have at least one Kubernetes feature gate associated. This is enforced by the API server when a node updates its status.
...
*   The `Discover` method for each feature receives a `NodeConfiguration` struct. This struct provides the context needed for a feature to determine if it's active. The key inputs are:
    *   **`FeatureGates`**: This allows the `Discover` method to check the status of any standard Kubernetes feature gates. Every registered feature should have at least one feature gate associated with it.
    *   **`StaticConfig`**: This input is optional. This struct contains static settings on the node which are not expected to change without a kubelet restart. These settings are derived from kubelets's command-line flags or the configuration file ((Kubelet config)[https://kubernetes.io/docs/reference/config-api/kubelet-config.v1beta1/]).
        *   Example -  `CPUManagerPolicy`: The value of the `--cpu-manager-policy` Kubelet flag. This setting along with feature gate `InPlacePodVerticalScalingExclusiveCPUs` can be used to determine if the node supports in-place CPU resizing for Guaranteed QoS pods.
...
type Feature interface {
	...
	Discover(cfg *NodeConfiguration) *DiscoveryResult
...
type DiscoveryResult struct {
	...
	Enabled bool
	// FeatureGates lists feature gate strings that needs to be enabled for the feature to be enabled.
	FeatureGates []string
	// NecessaryStaticConfig lists the required static configuration
...
*   Node groups with active node. CA samples a real node from the group to create a template.
...
In both cases, template inherits the `node.status.declaredFeatures` and The `NodeDeclaredFeatures` scheduler plugin will correctly consider this in the CA simulator.
...
*   By default, the node origin is set to `ClusterNode` and is used by the default kube-scheduler implementation. The `NodeDeclaredFeatures` scheduler plugin's behavior would remain unchanged for `ClusterNode`.
...
*   Unit coverage for the new changes in the shared library and the scheduler plugin.
