...
Dynamic Containers is a feature that lets *main* containers be added to and removed from a pod while it's running. By decoupling scheduling, initialization, and workload execution, this feature enables the Kubernetes Pod to become a highly dynamic execution envelope for high-churn, low-latency container operations.

This KEP describes a **building block** towards this eventual goal of high-churn, high-scale, low-latency operations. The initial goal is to unblock frameworks to start building around this model. Beyond the changes proposed here, more work will be needed to handle high-churn at scale and reduce latency, both of the API request and the underlying container operations. These separate work streams are already underway, and are out-of-scope for this KEP.
...
The scope of this KEP is deliberately minimized for more effective execution. I expect these non-goals to be addressed in subsequent KEPs at a later date.
  ...
  - Enabling dynamic volume management (aside from mounting preallocated volumes). See https://github.com/kubernetes/enhancements/issues/6395.
  ...
  - Optimizing and scaling the API and control plane.
...
Non-disruptive in-place upgrades of sidecar containers and system workloads.
  ...
  - _NOTE: The initial scope of this KEP does not include mutations to InitContainers, so this use
    case would require the "sidecar" to run as a main container.
...
Containers in the `Waiting` state are not ready, and currently cause the Pod to become unready. With
DynamicContainers the logic will be updated so waiting containers with the `Unallocated` reason are
ignored when computing pod readiness.

Once a container is allocated, it is still in the waiting state before it is started. When this
happens, the container **will** cause the pod to become unready. This is working-as-intended for the
initial scope of DynamicContainers, but will be addressed as part of a broader feature request to
exempt containers from pod readiness. See https://github.com/kubernetes/kubernetes/issues/127276.
...
Once a removed container is terminated, its `ContainerStatus` will be kept in the pod status (of the Kubernetes API). The Kubelet will preserve removed container statuses in the API when it updates the pod status. Garbage collection will be applied to removed container statuses.
...
- High error rate metrics:
  - `kubelet_sync_result_errors_total` (new)
  - `kubelet_pod_worker_errors_total` (new)
  - `kubelet_runtime_operations_errors_total`
- High sync operation durations:
...
1. `kubelet_sync_result_errors_total`
  - **Description:** Counter for `SyncResults` with an error attached. Incremented per `SyncResult` within `PodSyncResult`.
  - **Label:`action`** Corresponds to the [`SyncAction`](https://github.com/kubernetes/kubernetes/blob/c33bd535e902c775c6631f846d0e352742779d19/pkg/kubelet/container/sync_result.go#L109-L130) with the error, or `SyncPod` for top-level errors.
2. `kubelet_pod_worker_errors_total` - Corresponds to `kubelet_pod_worker_duration_seconds`
  - **Description:** Counter for [`podWorkerLoop`](https://github.com/kubernetes/kubernetes/blob/1f9e53d0ee0ed68b470bdb1b47397be80eec941b/pkg/kubelet/pod_workers.go#L1247) iterations with an error, excluding `context.Canceled` errors.
  - **Label:`operation_type`** Matches the `operation_type` recorded with `kubelet_pod_worker_duration_seconds`.
...
preferred. Here is a sample VAP that enforces immutability, except for on a set of allow-listed fields:
...
spec:
  ...
  matchConstraints:
    ...
    - apiGroups: [""]
      apiVersions: ["v1"]
      # Skip kubelet status updates, which cannot mutate spec.
      operations: ["UPDATE"]
      resources: ["pods/status"]
  variables:
    # Assigning spec to variables widens its static CEL type to `dyn`, enabling
    # dynamic map key iteration (.all) across all present (non-omitted) JSON fields.
    - name: oldSpec
      expression: oldObject.spec
    - name: newSpec
      expression: object.spec
    # Top-level PodSpec fields allowed to be added, modified, or removed.
    - name: mutablePodFields
      expression: "['tolerations', 'activeDeadlineSeconds', 'schedulingGates']"
    # Per-container fields allowed to be added, modified, or removed.
    - name: mutableContainerFields
      expression: "['resources']"
    # Container lists are validated separately in the second rule rather than by deep equality.
    - name: containerListFields
      expression: "['containers', 'initContainers', 'ephemeralContainers']"
    # Pair each container list's [oldList, newList], defaulting omitted lists to [].
    - name: containerListPairs
      expression: >
        variables.containerListFields.map(f, [
          f in variables.oldSpec ? variables.oldSpec[f] : [],
          f in variables.newSpec ? variables.newSpec[f] : []
        ])
  validations:
    # 1. PodSpec check: every non-allowlisted field in oldSpec must still exist and be
    # deeply equal in newSpec, and newSpec cannot introduce new non-allowlisted fields.
    - expression: >
        variables.oldSpec.all(k,
          k in variables.containerListFields ||
          k in variables.mutablePodFields ||
          (k in variables.newSpec && variables.newSpec[k] == variables.oldSpec[k])
        ) &&
        variables.newSpec.all(k,
          k in variables.containerListFields ||
          k in variables.mutablePodFields ||
          k in variables.oldSpec
        )
      message: "pod spec field is immutable"
      reason: Invalid
    # 2. Container check: forbid adding, removing, or reordering containers in any list,
    # and require each container's fields to remain equal except mutableContainerFields.
    - expression: >
        variables.containerListPairs.all(p,
          p[0].map(c, c.name) == p[1].map(c, c.name) &&
          p[0].all(oc, p[1].exists(nc, nc.name == oc.name &&
            oc.all(k, k in variables.mutableContainerFields || (k in nc && nc[k] == oc[k])) &&
            nc.all(k, k in variables.mutableContainerFields || k in oc)
          ))
        )
      message: "containers are immutable, except allowed fields"
      reason: Invalid
```
