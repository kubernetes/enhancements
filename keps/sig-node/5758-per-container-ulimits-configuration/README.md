...
2. Process Table Exhaustion (nproc)
   - Risk: The nproc limit is applied per-UID, not per-container. For example, 10 containers running as UID 1000 with nproc=1000 share a single pool of 1000 processes.
...
1. Validation and Limits:
   - Validation caps nofile at 1,048,576 to prevent excessive file descriptor usage.
   - Only privileged containers are allowed to set unlimited (-1) values.
...
2. Kubelet validation:

- If privileged container:
  - The Kubelet allows -1 to pass through without modification.
- If unprivileged container:
  - The Kubelet rejects the request with a validation error, as unprivileged containers cannot set -1 (unlimited) for ulimits.
- No value conversion occurs in the Kubelet.


3. Kubelet to CRI:
   - The Kubelet passes the -1 value unchanged in LinuxContainerSecurityContext.ulimits[].soft/hard to the CRI.

4. Container Runtime (CRI-O/containerd):
   - The container runtime receives the -1 value.
   - The runtime passes the -1 value to the OCI runtime spec unchanged.

5. OCI Runtime (runc/crun):
- The OCI runtime converts -1 to the RLIM_INFINITY constant.
- The OCI runtime calls the setrlimit(2) syscall with RLIM_INFINITY.

6. Linux Kernel:
- The Linux kernel maps RLIM_INFINITY to the resource-specific maximum value:
  - nofile: min(RLIM_INFINITY, /proc/sys/fs/nr_open) (typically 1048576).
  - nproc: min(RLIM_INFINITY, /proc/sys/kernel/pid_max).
  - Other resources: Effectively unlimited up to ULONG_MAX.

**Important Note**: The container process never sees the literal -1 value. For example, running ulimit -n inside the container will display the kernel-resolved maximum value.
...
We need to add a new field called `ContainerUlimits` to the `NodeRuntimeHandlerFeatures` in CRI.
...
- The nproc limit is applied per-UID, not per-container.
...
- Monitor node-level resources such as file descriptor count and process count.
- Use Pod Security Policies to restrict high ulimit values.
