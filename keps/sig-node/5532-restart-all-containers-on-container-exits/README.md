...
Yes, the upgrade->downgrade->upgrade path was tested manually
to ensure that the `RestartAllContainers`
...
The pod uses this feature if any of its containers specify `RestartPolicyRules`
with `RestartAllContainers` action, and the container exited with a matching exit code. 
If a pod is restarted by this feature, the container status will show the last termination status
as restarted by this policy.
The container restart count will also be incremented.

Additionally, the `kube_pod_container_status_restarts_total` metric
can indicate container restarts that might be governed by these rules.
...
- The time taken for restart all containers to succeed should be within the typical
container restart latencies (accounting for exponential backoff) multiplied by the number of containers.
