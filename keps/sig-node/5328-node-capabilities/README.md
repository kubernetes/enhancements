...
As part of the feature's graduation to GA, the Kubelet must stop reporting the capability. This will occur after the feature has been GA for a duration that exceeds the supported version skew. This ensures that obsolete information is automatically removed from `node.status` object.
...
*  Access to the central registry of capabilities. The library uses this to access the lifecycle metadata for each capability.
...
* kube-scheduler would call `InferPodCreateRequirements` once during the `PreFilter` stage and then call `MatchNode` for each node during the Filter stage.
...
3. Enqueue Extension:
    *   To make the scheduler responsive to cluster changes, the plugin implements queueing hints.
    *   It registers for `Node` `Add` and `Update` events. When a node is updated, a hint function checks if the node's capabilities have changed in a way that might satisfy a previously unschedulable pod. If the change is relevant, the pod is moved back to the active queue.
...
* Automated Deprecation (Post-GA): The reporting and checking of a capability are automatically disabled once a feature is old enough to be considered universally present. This logic is executed independently by the Kubelet and the control plane's shared library.
    ...
    * Control Plane: The shared library, using the version and FeatureGate object provided by the consuming component (e.g., kube-scheduler) at initialization, bypasses checks for the same capability. For example, if a feature goes GA in v1.38 and the skew is 3 versions, kube-scheduler running v1.42 or later will bypass that feature's capability check.
...
**Phase 1: Feature Development**
  * Kubelet Changes
    *  A new capability key is defined:`lifecycle.kubernetes.io/guaranteed-qos-pod-cpu-resize`. The Kubelet is updated to report this capability as "true" only if all underlying dependencies are met
        *  `InPlacePodVerticalScalingExclusiveCPUs` feature gates are enabled and CPU Manager Policy is set to `static`.
        * Or, CPU Manager Policy is `none`.
...
*   kube-scheduler (for the `NodeCapability` scheduler plugin):
    *  Integration tests would be added in `test/integration/scheduler/filters`. They will verify the scheduler plugin's PreFilter, Filter, and Enqueue logic by creating `v1.Node` objects, patching their `status.capabilities` field, and scheduling `v1.Pod` objects whose specs imply a requirement for those capabilities. Performance tests will be added in `test/integration/scheduler_perf`.
    *  Performance tests would be added in `test/integration/scheduler_perf`.
