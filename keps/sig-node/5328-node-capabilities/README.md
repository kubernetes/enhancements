...
This design ensures that the `NodeCapabilityFilter` plugin remains generic and does not require modification when new capabilities are added. All capability-specific inference and matching logic is encapsulated within the shared library.
...
This approach keeps the `NodeCapabilityValidator` plugin generic. It does not need to be updated when new capabilities are introduced, as all feature-specific logic resides in the shared library.
...
Node capabilities are checked by the scheduler during scheduling and then validated again by the Kubelet before a pod's containers can start. If a node's capability is removed (kubelet restart with a feature gate disabled) after the pod starts running, the pod is not automatically removed. Kubelet will re-validate the pod's requirements against the node's current state upon container or node restart. If a capability is now missing from the node, the pod's containers won't be started and the pod to enter a failed state.
...
*   In a cluster with `InPlacePodResizeExclusiveCpus` disabled and static CPU policy enabled, the admission controller should reject CPU resize requests for guaranteed QOS pods.
