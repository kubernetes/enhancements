...
3.  Validation Rules
    ...
    *   The list would be sorted alphabetically and would not contain duplicate entries.
...
Kubelet has the primary responsibility of discovering the enabled features relevant for control plane during its bootstrap and updating `node.status.declaredFeatures`. It determines its full, static set of declared features **once** upon startup based on its configuration. This list is held in memory and is included as a part of the periodic `NodeStatus` update that the Kubelet sends to the API server. The Kubelet is the authoritative source of truth for its own declared features. If an external controller or user were to modify the `node.status.declaredFeatures` field on the Node object, the Kubelet's next periodic status update would automatically overwrite and revert those changes.
