...
- [Design Details](#design-details)
  ...
  - [Future Considerations](#future-considerations)
    - [[Scoped for Beta] Plugin-Derived Capabilities](#scoped-for-beta-plugin-derived-capabilities)
...
This proposal introduces a new field `Capabilities `to` NodeSpec.NodeStatus `to expose information which the kube-scheduler and/or the admission controller would use to make more informed decisions.` `Kubelet is primarily responsible for discovering, consolidating, and reporting `Capabilities` to the API server. 

**Node Capabilities Requirements:**

1. Must be derived from static configuration.
...
### API Changes
 ...
 \
Add a field `Capabilities` field as type `map[string]string `to the` NodeSpec.NodeStatus` structure. 
...
1. If multiple interdependent settings are required to enable a feature, they must be collapsed into a single, logical capability. This simplifies the decision-making process for control plane components like the scheduler, which should not need to understand multiple interdependent settings. 
For example, Swap support on a node depends on several conditions: the feature gate, Kubelet configuration, and underlying kernel support. Instead of reporting these three states individually, Kubelet evaluates them and advertises a single, simple capability: kubernetes.io/feature/swapSupport: "true".
2. A capability should only be present if `node.status.capabilities` map if the feature is enabled and functional ('true` or another descriptive string if the capability is not boolean). If a feature is disabled (e.g., a beta feature gate is explicitly set to false) or not supported at all (e.g., on an older Kubelet version or an incompatible OS), the Kubelet must omit the corresponding capability key from the map entirely. This ensures there is consistency between a Node that doesn't know about the feature and one that has it disabled which simplifies the logic for control plane components.
...
*   Other API options are explored in the [Alternatives](#alternatives) section.
...
*   Inspect the PodSpec to infer a set of required NodeCapability key-value pairs. This inference is performed using imperative logic built into the scheduler plugin (without new, explicit capability request fields in the PodSpec).
...
*   If the filter plugin does not find any Node with a required capability, the pod would remain in a pending state with a `FailedScheduling` event which contians the reason.
...
1. When developing a new Kubelet feature Eg: [In-Place Pod Resize for Guaranteed QoS pods](https://github.com/kubernetes/enhancements/issues/5294), the version skew management strategy involves
    ...
    *   Updating the <code>NodeCapabilityValidator</code> to check if the node specified in the pod's <code>spec.nodeName</code> possesses this capability before the resize request is admitted.
2. An Admin enables  <code>InPlacePodVerticalScaling</code> and <code>InPlacePodVerticalScalingExclusiveCPUs</code> feature gates  and `static` CPU Manager Policy on a NodePool. 
...
3. The `NodeCapabilityFilter` in kube-scheduler inspects the `PodSpec`. Upon detecting the `spec.resources` field, it infers that the pod requires the `PodLevelResources` capability. The filter automatically rejects all Windows nodes that do not advertise this capability, ensuring the pod is only placed on a compatible Linux nodes.

### Changes on Existing Nodes

Node capabilities should generally represent mostly stable foundational configurations of the Node, not frequently changing real-time statuses. Node capabilities are primarily assessed by the kube-scheduler during pod scheduling. Post-scheduling, the Node Capability changes (e.g., kubelet restart with configuration or feature gate changes) should only be relevant for new workload. 
...
*   The kube-scheduler changes for Node Capabilities does not alter any logic related to the existing mechanisms. In such cases, the scheduling restrictions would be additive in nature i,e we need the Node and the Pod to satisfy both NodeCapability and the existing mechanism to be schedulable on the Node. 
...
*   Kube-scheduler (for the `NodeCapabilityFilter` plugin):
    *   Verify that the plugin correctly parses the `node.status.capabilities` field from Node objects.
...
*   In a cluster with <code>[InPlacePodResizeExclusiveCpus](http://kubernetes.io/feature/inPlacePodResizeExclusiveCpus)</code> disabled, the admission controller should reject CPU resize requests for guaranteed QOS pods.
...
*   This presents a challenge when existing features begin to be reported through the new Node Capabilities mechanism. Few possible options to manage this are
    1. Implement phased capability reliance in the Control Plane. The newer (v1.X) control plane would initially avoid basing decisions based on the new capability signals, and would begin to do so past the allowed version skew (v1. X + version skew).
    2. Backfill capability information on older versions.
...
For Node Capabilities that report the status of specific feature gates, reporting must continue after the feature graduates to GA to support the version skew policy. This ensures a control plane can correctly manage older nodes where the feature may not be available. After the feature's GA version + supported version skew relase, the feature would become universally available on all the nodes in the cluster and at that point, the Kubelet can stop reporting and the Kube-scheduler can stop consuming the capability. 

### Future Considerations 

#### [Scoped for Beta] Plugin-Derived Capabilities

While the initial scope of this KEP is intentionally limited to static capabilities, a key future enhancement is to extend this framework to allow dynamically registered plugins (e.g., device plugins) to advertise their own stable capabilities. This would allow the scheduler to make even more informed decisions based on the full set of features available on a node, including those provided by add-on components.
...
Yes. Introducing new capabilities can prevent pod scheduling or pod request admission when the Nodes cannot satisfy the pod requirements. 
*Note:* Enabling the NodeCapabilities feature gate alone doesn't alter default behavior; it only activates the underlying framework. Actual changes to behaviors like pod scheduling or admission happen once nodes start publishing specific capabilities (for instance, Kubelet reporting a feature gate's status as a capability) and control plane components are updated to consume and act on this information.

###### Can the feature be disabled once it has been enabled (i.e. can we roll back the enablement)?

Yes, the feature can be disabled, but a specific rollback order must be followed to prevent scheduling disruptions. The feature must be disabled on Control Plane components before being Disabled on nodes. When the feature is disabled on a Node, the Kubelet would clear the NodeCapabilities field from the Node's status. Existing pods that are already running are unaffected by this process.

###### What happens if we reenable the feature if it was previously rolled back?

If the feature is re-enabled after being previously disabled, any new pod scheduling or request validation will again start considering Node Capabilities. The feature should be enabled on the Nodes before it is enabled on the Control Plane to ensure the scheduling decisions are not affected.
...
* Validate that the Kube-Scheduler plugin filtes Nodes based on `node.status.capabilities` when the feature is enabled, and ignores the field when the feature is disabled.
