...
1. The feature developer will enhance the Kubelet to conditionally advertise capabilities for `PodLevelResources`. Kubelet will check that both the feature gate is enabled and the underlying OS is Linux before adding the `kubernetes.io/feature/PodLevelResources: "true"` capability to the node's status.
...
*   In a cluster with <code>I[nPlacePodResizeExclusiveCpus](http://kubernetes.io/feature/inPlacePodResizeExclusiveCpus)</code> disabled, the admission controller should reject CPU resize requests for guaranteed QOS pods.
...
*   This presents a challenge when existing features begin to be reported through the new Node Capabilities mechanism. Few possible options to manage this are
    ...
    3. Use a "default-allow" strategy when the capability is absent. This maintains the current kube-scheduler behavior for these pre-existing features. Nodes are filtered out only if the capability is explicitly set to false.
...
Yes, the feature can be disabled to revert to the status quo. When disabled, scheduling decisions and API request validations will no longer utilize the Node Capabilities information that this feature enables. Existing pods that are already running should be unaffected.
