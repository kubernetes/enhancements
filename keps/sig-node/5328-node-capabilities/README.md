...
- [Design Details](#design-details)
  ...
  - [Future Considerations](#future-considerations)
    ...
    - [Cluster Autoscaler Integration](#cluster-autoscaler-integration)
      - [Approach 1: Inferring Capabilities from a Live Node](#approach-1-inferring-capabilities-from-a-live-node)
      - [Approach 2: Capability information in Node Group Template](#approach-2-capability-information-in-node-group-template)
...
##### Approach 1: Inferring Capabilities from a Live Node

In this approach, the Cluster Autoscaler would inspect one of the running nodes in a target node group (if one exists). Its internal simulator would then use the node's `node.status.capabilities` and the shared capability matching library to determine if a new node from that group would be a valid match for the pending pod.

* Drawbacks: 
    * Fails to work in Scale-from-Zero Scenarios: If there are no active nodes created with the NodeGroup template, the CA does not know about node capabilities.
    * Inconsistency During Rolling Upgrades: During a version upgrade, a node group temporarily contains a mix of old and new nodes with different
      capabilities. The CA's decision would become non-deterministic; if it inspects an old node, it will fail to scale up, and if it inspects a new
      node, it may scale up, but the pod could still land on an old, incompatible node.

##### Approach 2: Capability information in Node Group Template

A long-term solution to solve the Scale-from-Zero problem requires that capability information be made available as part of the node group's template or a separate CRD (as proposed in [kubernetes/autoscaler#7799](https://github.com/kubernetes/autoscaler/issues/7799)), providing an authoritative signal to the CA.
