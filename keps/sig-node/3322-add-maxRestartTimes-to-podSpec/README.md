...
- Test cases:
  - tests with `Job`'s `backoffLimit` and Pod's `maxRestartTimes` both set
...
- Test cases:
  - pod lifecycle with restartPolicy set to `Always` or `Never`
  - pod lifecycle with restartPolicy set to `OnFailure`
