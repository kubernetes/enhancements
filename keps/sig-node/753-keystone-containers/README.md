...
-->
# KEP-2872: Keystone Containers KEP
...
There are lots of [user requests](https://https://github.com/kubernetes/kubernetes/issues/25908) for better support of sidecar containers in Jobs but this issue mostly gets lost in a larger attempt to address the sidecar issue as a whole.

Consider a K8s job with two containers one that does the actual work and another one is for monitoring/logging, the keystone container will complete the work and exit, the sidecar container will keep running and the job will never run to completion. This is problematic for any external application/tool depending on the job waiting to gets completed. Also, the sidecars unnecessarily occupy the system resources.

The only workaround is that the sidecar container needs some kind of signaling from the app container when it exits or the sidecar monitors the app container status. This is not the ideal workaround as it requires containers to be platform aware.

Another motivation for this KEP is to take small steps towards the sidecar containers that will help a lot of users without including complicated logic and changing critical kubelet code paths.
...
### Goals
- Pod termination by kubelet if application containers have successfully run to completion and exited.
...
- Address sidecar containers issues at once with any of the following proposals or build a new proposal with community consensus.
    - [pod lifecycle phases proposal ](https://docs.google.com/document/d/1Q3685Ic2WV7jPo9vpmirZL1zLVJU91zd3_p_aFDPcS0/edit#heading=h.1di6ei9de9rm) by Tim Hockin 
    - [Explicit container dependencies proposal](https://docs.google.com/document/d/15Ker0cm6n3auAy_lYmvthilhTxF8OVisjl_52nTQJBE/edit#heading=h.g63xga7g2qzb) by Rodrigo Campos 
