...
The PodResizePending condition should set as Infeasible if CPU static policy resize feasibility check fails:
   ...
   * kubelet fail to GetBaselineCPUSet from CPU Manager state file, thus not making it possible to perform a feasibility check, with `getBaselineCPUSetError` as reason
...
For the problems solved by the proposed `Baseline` introduction in checkpoint, the following options have been cosidered and presented in sig-node meetings. 
