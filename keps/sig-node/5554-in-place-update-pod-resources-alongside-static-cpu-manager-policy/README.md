...
- [CPU Manager pod resize, with InPlacePodVerticalScaling enabled, InPlacePodVerticalScalingExclusiveCPUs disabled](https://github.com/esotsal/kubernetes/blob/aae0b5f8156637230259cef75070b9fb49031f9d/test/e2e_node/cpu_manager_test.go#L2058-L2413)
...
- Upon success, the user can check the CPU affinity of the resized Pod, from within the running container to confirm assigned CPU set is increased keeping promised CPU set.
...
Yes, usage metrics and/or podresources API (or any other suitable APIs) and/or surface data in the statuses ( planned for Beta ) would be useful to improve observability allowing user to determine the feature is functioning properly. 
