...
- [Design Details](#design-details)
  - [Implementation](#implementation)
    ...
    - [Promised CPUs checkpoint](#promised-cpus-checkpoint)
...
- [Alternatives](#alternatives)
  ...
  - [Option 2: Use a LIFO in cpu assignment](#option-2-use-a-lifo-in-cpu-assignment)
<!-- /toc -->
...
This proposal aims to implement this capability, which was a planned (but out-of-scope for beta) enhancement for the In-Place Resource Resize feature.
...
* Support CPU limit decrease without container restart up to the originally configured CPU values (during pod creation on the Node) keeping originally configured CPU NUMA affinity.
* Support CPU limit increase without container restart keeping originally configured CPU values (during pod creation on the Node) keeping originally configured CPU NUMA affinity.
...
[KEP-693](https://github.com/kubernetes/enhancements/tree/master/keps/sig-node/693-topology-manager) introduced Topology Manager in `kubelet` for topology aligned resource allocation choices for a container. `TopologyManagerPolicyOptions` allows the selection of topology manager allocation policies. As for kubernetes 1.33, the following policies are available:
...
In the below two sections we present sequence diagrams during Pod Admission and current behaviour upon an InPlacePodVertical Scaling attempt of a Guaranteed QoS PoD with static CPU Manager policy enabled ( Kubernetes v1.33 is used at the time of writing of this KEP )
...
* Must not allow decrease of number of CPUs below the CPUs allocated upon creation of the Guaranteed PoD (“promised”) for the running container in question, instead infeasible error should be returned.
...
* Same rules for NUMA Topology applicable for CPU Limit increase should apply for CPU Limit decrease ( Current behaviour with Pod Admit )

### Use Cases for CPU Resize

In the following section we will present examples demonstrating the proposed behaviour based 
...
It is in scope of this KEP to create tests with all possible combinations to test and verify expected behaviour is ensured for all combinations as well as negative tests.
...
When the topology manager fails to generate a valid topology hint, the pod resize is marked as [Deferred](https://github.com/kubernetes/enhancements/tree/master/keps/sig-node/1287-in-place-update-pod-resources#resize-status). This means that while the node theoretically has enough CPU for the resize, it's not currently possible but can be re-evaluated later, potentially becoming feasible as pod churn frees up capacity.

Reasons for failure 
* CPU manager cannot generate topology hints. This would happen only during CPU increase resize request if:
  + the current NUMA affinity can accommodate the increase 
...
Three additional Error constants, to cater Error handling of such corner cases and to facilitate troubleshooting during production, specific to CPU Manager InPlacePodVerticalScaling failures were needed. 

Please refer to [Introduced Error types for this KEP](#introduced-error-types-for-this-kep) for more details.

### Current behavior

In-place resource resizing is infeasible for Guaranteed QoS pods on nodes configured with a Static CPU Policy ( issue [#127262](https://github.com/kubernetes/kubernetes/issues/127262) ). Any attempt of in-place resize for Guaranteed QoS pods with static CPU policy will return false and update status to v1.PodResizeStatusInfeasible.
...
With this KEPs implementation when `InPlacePodVerticalScalingExclusiveCPUs` and `InPlacePodVerticalScaling` feature gates are both enabled in-place resize for GuaranteedQoS pods with static Policy should be possible.
...
The modifications in this file are only in the AddContainer method and related with the addition of PromisedCPUSet in CPU Managers checkpoint. 

Updates in cpu_manager_test.go have been implemented.

#### policy_static.go modifications

This is the core implementation where it is handled the resize of a Guaranteed PoD with CPU Static policy. It is worth mentioning: 
* the introduction of  a new method validateInPlacePodVerticalScaling which assures QoS will be kept the same otherwise it will return error
...
The following three Error InPlacePodVerticalScaling constants are introduced for this KEP
...
These cases will be added in the existing integration tests:
    ...
    InPlacePodVerticalScalingExclusiveCPUs works as expected.
...
Addition of CPUManager checkpoint version 3 doesn't impact an upgrade or downgrade, since CPU Manager current behaviour is to create a new checkpoint upon kubelet start, otherwise it will require the Node to be drained, i.e. to delete /var/lib/kubelet/cpu_manager_state file. 
...
* [x] Feature gate (also fill in values in `kep.yaml`)
  ...
  + Components depending on the feature gate: kubelet, kube-controller-manager, kube-scheduler
    - kubelet
  + Requires `InPlacePodVerticalScaling` and `CPUManager` feature to be enabled
...
Yes. Admission flow changes for In-place CPU resize requests for Guaranteed QOS pods. Kubelet will start processing these requests and adjust the CPU Manager's internal state and potentially the CPU and/or NUMA affinity cgroup to apply the resize.
...
2025-09-21: initial KEP draft created
...
For the problems solved by the proposed `promised` introduction, the following options have been cosidered:

### Option 1:New field mustKeepCPUs in API

[TODO ESOTSAL check signode meeting notes/video and update]
...
Cons:
having in the beginninga mustKeepCPUs value to be introduced in API but it was 

### Option 2: Use a LIFO in cpu assignment

This
We considered having a LIFO functionality introduced in cpu_assignement 
