...
exit codes of the container, representing the restart of a task, to be treated
as non-failure even if the Pod has a restartPolicy=Never. This scenario is
...
configured with `restartRules` to restart in-place for training errors.
...
with a new field `ContainerRestartrRules`. A list of rules will be evaluate
in order to determine the action of container exit based on the conditions.
...
`restartRules` has no effect because the only action is `Restart`.
...
No. The feature introduces a new API field, restartRules, to the container
...
restartRules field while the gate was enabled, those rules will be ignored by
...
the restartRules for any Pods that have the field defined. The container
...
Enabling this feature will introduce a new field `restartRules` on the
