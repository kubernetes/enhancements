...
A clearer defintion of pod restart status should be delegated to the kubelet
from the controllers. This proposal is also designed to address the gap in
Job `podRestartPolicy`. Currently `podRestartPolicy` only supported
`restartPolicy=Never`, and container in-place restarat will not be considered
a failure of the pod.
...
allowed values. The shape of API is informed by some future improvements
...
can be configured to be restartable.

For the pod with the `restartPolicy=Never`, it will allow restarting
the container for the subset of exit codes.
...
with the restart count higher than 0. It was never possible before.
