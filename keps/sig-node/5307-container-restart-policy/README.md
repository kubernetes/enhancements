...
- Introduce API which allows to keep restarting a container on specified exit codes.
...
for a better utilization of compute resources.
...
As a Kubernetes orchestrator, I would like to differentiate between retriable errors
via container exit codes. I would like to restart the container in-place if the error
is retriable to avoid re-scheduling the Pod for shorter pod/container restart time,
and I would like to reschedule the Pod if the error is not retriable.
...
may have containers restarted and have the restart count higher than 0. 
...
kubelet if more suitable to handle the container restart policy. This aligns
...
configured with `restartPolicy` to restart in-place for training errors.
...
The proposal is to extend the Pod's [ContainerAPI](https://github.com/kubernetes/kubernetes/blob/master/pkg/apis/core/types.go#L2528),
especially the ContainerRestartPolicy with a new possible value of "RuleBased", and
introduce a new type of ContainerRestartRules with only one allowed Action, "Restart.
...
const (
  ...
  // New "RuleBased" ContainerRestartPolicy
  ContainerRestartPolicyRuleBased ContainerRestartPolicy = "RuleBased"
)

type Container struct {
  // Omitting irrelevant fields...

  // Currently only "Always" is allowed for init containers; no value
  // is allowed for normal containers. With this KEP, "RuleBased" will
  // be allowed for both normal and init containers. The Container
  // Restart Policy will override the Pod's restart policy.
  RestartPolicy *ContainerRestartPolicy

  // Can only be specified if the RestartPolicy is not "RuleBased".
  ...
  // is restarted or not.
  RestartRules []ContainerRestartRule
}
...
type ContainerRestartRule struct {
  ...
  OnExitCodes ContainerRestartRuleOnExitCodesRequirement
}
...
// ContainerRestartRuleOnExitCodesRequirement describes the requirement
// for handling an exited container based on its exit codes.
type ContainerRestartRuleOnExitCodesRequirement struct {
...
With the limitations above, an API will do nothing for pods with
pod-level `restartPolicy=Always`, as the only container-level restart
rules action is `Restart`. Same for the pods with pod-level 
