...
- [Design Details](#design-details)
  - [API Changes](#api-changes)
    ...
    - [Special Cases](#special-cases)
      - [Handling Multiple Claims for the Same Native Resource with Different Accounting Policy](#handling-multiple-claims-for-the-same-native-resource-with-different-accounting-policy)
...
const (
  // AccountingPolicyOverride means the DRA claim replaces the pod spec request.
  AccountingPolicyOverride NativeResourceAccountingPolicy = "Override"
  // AccountingPolicyAddPerClaim means the DRA claim is added once for each claim.
  AccountingPolicyAddPerClaim      NativeResourceAccountingPolicy = "AddPerClaim"
  // AccountingPolicyAddPerReference means the DRA claim is added for each container referencing the claim.
  AccountingPolicyAddPerReference NativeResourceAccountingPolicy = "AddPerReference"
  // AccountingPolicyConsumeFrom means the container's standard requests are drawn from the pool defined by the DRA claim.
  AccountingPolicyConsumeFrom      NativeResourceAccountingPolicy = "ConsumeFrom"
)
...
*   **`DeviceClass.spec.ManagesNativeResources`**: If true, it signals to the scheduler's `NodeResourcesFit` plugin that devices of this class have native resource mappings, and the node resource fit check for all native resources for pods using this class should be handled by the `DynamicResources` plugin.
...
*   **`Device.spec.nativeResourceMappings`**: This new struct within `Device` object in a `ResourceSlice` provides the specific details of how this particular device instance relates to native resources.
    ...
    *   **AccountingPolicy:** Defines how the native resource quantity from the DRA claim is accounted for along with any requests for the same resource made in the container's `resources.requests` in the Pod Spec.
        *   **Override:** The quantity from the DRA claim fully replaces any request in the PodSpec for the same resource . 
        *   **AddPerClaim:** The quantity from the DRA claim is added to the standard request for the resource in the pod spec. If the claim is shared by multiple containers in the same pod, the request in the claim is added once to the Pod's total requests for this resource.
        *   **AddPerReference:** The quantity from the DRA claim is added to the standard request for the resource in the pod spec. If the claim is shared by multiple containers in the same pod, the request in the claim is added once every time the claim is referenced.
        *   **ConsumeFrom:** A DRA claim is defined to represent the native resource pool capacity. All the containers or pods referencing the claim are satisfied from the capacity pool defined by the DRA claim. Pods access this pool by referencing the corresponding `ResourceClaim` in their spec.`containers[].resources.claims`. The scheduler ensures that the sum of requests from all containers sharing this claim on a node does not exceed the pool's capacity. The entire pool capacity reserved on the node, making it unavailable for other pods outside this pool.
...
*   The device represents a pool of 128 CPUs represented as a consumable device.
*   `AccountingPolicy: "ConsumeFrom"` indicates pods with ResourceClaim with this device assigned will draw from this pool.
...
1.  **PreFilter Stage:** This stage is for initial checks and pre-computations to quickly filter out non-viable nodes or prepare data for the `Filter` stage, minimizing work on each node.
    *   **NodeResourcesFit Plugin**: It examines the `DeviceClass` from `pod.spec.resourceClaims`. If `DeviceClass.Spec.ManagesNativeResources` is true for any claim, it defers all native resource checks for this pod to the `DynamicResources` plugin.
...
4.  **Bind Stage (Framework Cache Update):** This is the final step where the scheduler records the pod's resource consumption on the node. The core logic in the scheduler framework is enhanced to use `pod.Status.NativeResourceClaimStatus` and new fields in `NodeInfo` to track DRA claim states.
  ...
  To manage the complex accounting requirements introduced by different `AccountingPolicy` values, the `NodeInfo` struct is extended. The `DRAClaimStates` map tracks the state of each DRA claim active on the node. This allows the scheduler to handle shared claims between different containers and pods on the node.
...
##### Handling Multiple Claims for the Same Native Resource with Different Accounting Policy

A Pod may reference multiple `ResourceClaim`s with `NativeResourceMappings` and these claims might be for different device classes. The scheduler must have a clear set of rules for how to aggregate the native resource requests from these different claims when they are associated to the same resource and have different `AccountingPolicy`.

*  For any given resource within a single Pod, all `NativeResourceMapping` entries with `Override` policy across all devices allocated for the Pod must be for the same device (Eg: devices published by CPU DRA driver). The `DynamicResources` plugin will throw an error in the `Filter` stage if we have devices from different drivers attempting to `Override` the same native resource.
*  A container can only have its request for a specific native resource satisfied by one `ConsumeFrom` pool. Multiple claims with `ConsumeFrom` for the same resource cannot be used by the same container. This is enforced during `Filter` stage. 
*  A container cannot have multiple claims with different accounting policies for the same resource. 
*  Claims with `AddPerClaim` or `AddPerReference` policy are additive. Their quantities are summed together with the request in the pod spec, or the claim with `override` policy.
...
Valid Case:
  ...
  * Claim 1: Uses DeviceClass "A". Device mapping: {Resource: cpu, AccountingPolicy: Override, Quantity: 4 CPU}
  * Claim 2: Uses DeviceClass "A". Device mapping: {Resource: cpu, AccountingPolicy: Override, Quantity: 8 CPU}
  * Claim 3: Uses DeviceClass "B". Device mapping: {Resource: cpu, AccountingPolicy: AddPerClaim, Quantity: 2 CPU}
  ...
  Resource Footprint:
    ...
    * Claim 3 has `AddPerClaim` which makes it 14 CPU (12 + 2)
...
*   If `AccountingPolicy` is `AddPerClaim`: The native resource quantity from the claim's `NativeResourceMapping` is accounted for only once for the entire pod.
*   If `AccountingPolicy` is `AddPerReference`: The native resource quantity is counted each time a claim is referenced by a container.

**Example:**

Pod with Container A and Container B both using `claimX`.
`claimX` maps to a device with `NativeResourceMapping`: {ResourceName: "cpu", QuantityFrom: {PerInstanceQuantity: "2"}, AccountingPolicy: AddPerReference}

*   Effective CPU overhead from `claimX` = 2 + 2 = 4 CPU. This is added to other CPU requests.
If `AccountingPolicy` was `AddPerClaim`, the overhead would be just 2 CPU.
...
**Problem with Current KEP Proposal:**
