...
- **API Server Validation**: In
  ...
  - Verify field is dropped when feature gate is disabled, and preserved if
    already in use.
- **ResourceSlice Controller Helper Unit Tests**: In
  `staging/src/k8s.io/dynamic-resource-allocation/resourceslice/resourceslicecontroller_test.go`:
  - Verify that `SkipNodeOperations` is synced to `ResourceSlice.Spec.SkipNodeOperations`.
  - Verify that a `DroppedFieldsError` is returned when `SkipNodeOperations` is
    dropped by the API server because `DRAOptionalNodeOperations` is disabled.
...
- Test Case 1.3: Missing node component failure when skipping only unprepare
  (`skipNodeOperations: ["NodeUnprepareResources"]`)
  - **API Configuration**: Publish `ResourceSlices` with `skipNodeOperations:
    ["NodeUnprepareResources"]`.
  - **Workload**: Deploy a Pod referencing this resource.
  - **Assertions**:
    - The Pod gets stuck in `ContainerCreating` with
      `FailedPrepareDynamicResources` errors because `NodePrepareResources` is
      still required and no node-local driver is running.

###### Scenario 2: Driver with node-local components (Standard Driver)
This scenario validates that the kubelet selectively invokes or bypasses
node-local driver gRPC operations according to `skipNodeOperations` when a
node-local driver is running.
...
- Test Case 2.2: Fully bypassed node operations (`skipNodeOperations: ["*"]`)
  - **API Configuration**: Publish `ResourceSlices` with `skipNodeOperations: ["*"]`.
  - **Workload**: Deploy a Pod.
  - **Assertions**:
    - The Pod reaches the `Running` phase.
    - Assert that the driver's `NodePrepareResources` **was not** called.
    - Delete the Pod.
    - Assert that the driver's `NodeUnprepareResources` **was not** called.
- Test Case 2.3: Skipping only unprepare (`skipNodeOperations: ["NodeUnprepareResources"]`)
  - **API Configuration**: Publish `ResourceSlices` with `skipNodeOperations: ["NodeUnprepareResources"]`.
  - **Workload**: Deploy a Pod.
  - **Assertions**:
    - The Pod reaches the `Running` phase.
    - Assert that the driver's `NodePrepareResources` **was** called.
...
- **Test Case 3.2: Feature Gate Rollback / Downgrade (N to N-1)**:
  ...
  - **Action 1 (Control Plane Downgrade)**: Downgrade the control plane to N-1
    ...
    - **Assertions**:
      - The API server allows the existing
...
- [x] Feature implemented behind the `DRAOptionalNodeOperations` feature flag (off
  by default).
- [x] Full unit and basic E2E test suites (Scenario 1 & 2) implemented and green.
- [x] Metrics (`dra_node_prepare_skips_total` and
  `dra_node_unprepare_skips_total`) added.
- [x] User and driver developer documentation published on
  [kubernetes.io](https://kubernetes.io/docs/concepts/resource-management/dynamic-resource-allocation/dra-features/#optional-node-operations).
...
- **Downgrade**:
  - **Feature Gate Rollback (on v1.37+)**: If the `DRAOptionalNodeOperations`
    feature gate is disabled on a kubelet that supports the feature:
    - New pods or restarting pods whose claims specify `skipNodeOperations` will
      fail `PrepareResources` with a `DRAOptionalNodeOperations feature gate is disabled on kubelet`
      error rather than running with unprepared devices.
    - Already-running pods keep their checkpointed `ClaimInfo` state (including
      the cached `skipNodeOperations` set), so `UnprepareResources` continues to
      skip cleanup and pods can be deleted cleanly without hanging in `Terminating`.
  - **Kubelet Binary Downgrade (to pre-v1.37)**: If a kubelet binary is
    downgraded to an older version that lacks `DRAOptionalNodeOperations`
    support altogether, it ignores the unknown `skipNodeOperations` field in the
    checkpoint and defaults to invoking node-local gRPC calls. If any pods are
    running using a driver without node-local components, those pods will fail
    to restart or delete cleanly. Operators must ensure all pods using no-prep
    claims are terminated before downgrading kubelet binaries to pre-v1.37, or
    run temporary no-op drivers during the downgrade transition.
...
However, devices in lingering `ResourceSlices` that still have
`skipNodeOperations` set (preserved by `kube-apiserver` across rollback) will be
treated as unallocatable (filtered out) by the allocator. If no other compatible
devices satisfy a claim, the pod remains `Pending` (unschedulable) rather than
being bound to a node where `skipNodeOperations` would either be stripped from
the new `ResourceClaim` allocation by `kube-apiserver` (causing the kubelet to
call a non-existent node-local driver) or rejected by `PrepareResources` on a
kubelet with the feature gate disabled.
...
- **Older or custom allocator fails to copy field**: If an older or custom scheduler/allocator does
  ...
  - *Mitigation*: Ensure the custom allocator/scheduler is upgraded to support and copy the new field
...
4. If the pod remains `Pending` (unschedulable) when requesting devices from a
   `ResourceSlice` with `skipNodeOperations` set, verify that:
   - Target nodes declare `DRAOptionalNodeOperations` in `node.status.declaredFeatures`
     (requiring `DRAOptionalNodeOperations` to be enabled on the kubelet).
   - The `DRAOptionalNodeOperations` feature gate is enabled in `kube-scheduler`
     (when disabled, the allocator treats devices with `skipNodeOperations` as
     unallocatable).
