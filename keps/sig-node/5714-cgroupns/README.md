<!--
Last template update: 2025-11-21

https://github.com/kubernetes/enhancements/commit/6ab9bf717d1228928740bdbfe761b6e62b870902
...
- [Proposal](#proposal)
  ...
  - [User Stories (Optional)](#user-stories-optional)
    - [Story 1: BuildKit](#story-1-buildkit)
...
- Add `CgroupNamespaces *bool` to [`NodeRuntimeHandlerFeatures`](https://pkg.go.dev/k8s.io/api/core/v1#NodeRuntimeHandlerFeatures)
...
will have to be updated to recognize `HostCgroupNamespace`.
...
N/A (other namespace options are not covered in integration tests too)
...
<https://github.com/kubernetes/kubernetes/blob/master/test/e2e/common/node/security_context.go>
will be updated to test the following configurations:
- `privileged: false`, `hostCgroupNamespace: false`
- `privileged: false`, `hostCgroupNamespace: true`
- `privileged: true`, `hostCgroupNamespace: false`
- `privileged: true`, `hostCgroupNamespace: true`

Similar tests will be added to <https://github.com/kubernetes-sigs/cri-tools/blob/master/pkg/validate/security_context_linux.go> too.
...
- Feature implemented in containerd, CRI-O, and cri-tools

#### GA

- Two releases after Beta
...
- [X] Feature gate (also fill in values in `kep.yaml`)
  - Feature gate name: `CgroupNamespacesSupport`
...
Unit tests:
- `pkg/api/pod`
  - With the gate disabled: On `dropDisabledFields`, the `HostCgroupNamespace` field is cleared on creating but preserved on updating
  - With the gate enabled:  The field is preserved on creating/updating
- `pkg/kubelet/kuberuntime`:
  - With the gate disabled: kubelet sends `CgroupnsOptions = nil` to the CRI runtime regardless to the `HostCgroupNamespace` configuration
  - With the gate enabled:  kubelet sends non-nil `CgroupnsOptions` to the CRI runtime
...
A specific version of CRI, with `CgroupNamespaces` present in
[`RuntimeHandlerFeatures`](https://pkg.go.dev/k8s.io/cri-api/pkg/apis/runtime/v1#RuntimeHandlerFeatures).
...
If any pod is facing the limit of the number of the cgroup namespaces, it should have been already
failing due to the limit of the number of the other namespaces (e.g., mount namespaces).
...
- 2026-12-XX: Kubernetes v1.38 (Alpha)
...
A user-provided entrypoint script may execute `unshare --cgroup` and `mount -t cgroup2 none /sys/fs/cgroup`,
but it can be complicated. See <https://github.com/moby/buildkit/pull/6368>.
