...
An operator should be already aware whether they are going to run kubelet in UserNS,
when they are adding a new node to the cluster.

For confirmation, they can run the following command to see if kubelet is running in UserNS.
...
If the result of the `readlink` command above is other than `user:[4026531837]`,
kubelet is running in UserNS.
...
[`NodeSystemInfo`](https://pkg.go.dev/k8s.io/api/core/v1#NodeSystemInfo) could have `RunningInUserNS *bool`
for better observability.

However, this does not seem necessary, as an admin should have been already aware whether they are going to setup kubelet in UserNS
when they are adding a new node to the cluster.
