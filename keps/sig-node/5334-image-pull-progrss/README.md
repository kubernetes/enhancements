...
- Implement support for the image pull progress API in containerd's CRI integration.
...
A user deploys an application to a Kubernetes cluster. As the Pod is scheduled and created,
...
CRI implementations may report image pull progress at different levels of detail: either for the entire image or for individual layers. 
...
- When reporting progress for the entire image, the `Name` field should be set to the image name.
- When reporting progress per layer, the `Name` field should be set to the layer's digest.

Implementers can choose either approach.

Implementers also determine the frequency of pushes.


Proposed changes to the crictl:

- Add a new flag to `crictl pull` that allows users to monitor the image pull progress.
...
- Add a new route `/imagePullProgress/{podNamespace}/{podID}` to the Kubelet server.
...
It is expected to be implemented on Alpha 2 or Beta

- Add a new subcommand `kubectl image-pull-progress <pod>` to allow users to monitor the image pull progress of a pod.
...
- Add support for image pull progress in kubectl
