...
- When reporting progress for the entire image, the `Name` field should be set to the image full name.
...
1. Individual Progress Tracking:
   - Maintain progress map keyed by Name
   - On each update:
     - Update map entry
     - Render individual progress indicators
   - Ideal for detailed multi-layer monitoring

2. Aggregated Progress Tracking:
   - Maintain progress map keyed by Name
   - On each update:
     - Update map entry
     - Calculate: `( sum(Progress) / sum(Total) ) * 100`
     - Show single progress indicator
   - Ideal for simplified overview
...
- Add a new subcommand `kubectl image-pull-progress <pod-name>`  in kubectl
...
- APIServer: Requires no modifications as it leverages existing proxy capabilities to route requests to kubelets
