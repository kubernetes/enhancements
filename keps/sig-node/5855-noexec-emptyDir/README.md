...
This feature involves coordination between the kube-apiserver (API validation), the kubelet (CRI passthrough and runtime feature detection), and the scheduler (node filtering via Node Declared Features). The following version skew scenarios are possible during rolling upgrades:

**Apiserver ON, kubelet OFF:** The apiserver accepts `bindMountOptions`. The kubelet does not declare `VolumeBindMountOptions` in `node.status.declaredFeatures`, so the scheduler avoids placing the pod on this node. If the pod is somehow scheduled (e.g., custom scheduler), the kubelet rejects it with an error, preventing silent degradation.

**Apiserver OFF, kubelet ON:** The apiserver does not know about `bindMountOptions` or strips the field when the gate is disabled. The kubelet never sees the field and behaves as before.
...
**Both ON, runtime supports `mount_options`:** Full enforcement. The kubelet declares `VolumeBindMountOptions` in `node.status.declaredFeatures`, the scheduler places pods on compatible nodes, the apiserver validates, and the kubelet passes `bindMountOptions` through CRI to the runtime.

**Both OFF:** Feature disabled, existing behavior.

In clusters with mixed node versions, the Node Declared Features framework mitigates version skew automatically. Only nodes that declare `VolumeBindMountOptions` receive pods with `bindMountOptions`, preventing kubelet rejections that would put the pod into a non-retriable `Failed` state.
