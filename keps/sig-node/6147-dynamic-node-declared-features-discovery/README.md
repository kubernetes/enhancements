...
Dedicated e2e tests are out of scope for this KEP. The behavior requires an
in-place container runtime upgrade while the kubelet and workloads remain
running, which is not supported by the current e2e test infrastructure. The
Alpha implementation will therefore be tested manually by:

- upgrading the container runtime with the feature enabled and confirming that
  `node.status.declaredFeatures` is updated without restarting the kubelet;
- restarting the kubelet with the feature enabled and confirming that running
  Pods continue to run while newly admitted Pods are still checked; and
- disabling and re-enabling the feature to verify the rollback and re-enable
  paths.

The manual test procedure and results will be documented before Beta.
...
Setting `NodeDeclaredFeaturesRuntimeDiscovery=false` and restarting the kubelet
reverts runtime feature discovery to startup-only. With `NodeDeclaredFeatures`
still enabled, all Pods are re-admitted against the current declared feature set
(KEP-5328 behavior). Running Pods requiring a feature the node no longer declares
can transition to `Failed`.

Downgrading to a kubelet version that predates the
`NodeDeclaredFeaturesRuntimeDiscovery` feature gate also restores this behavior
if `NodeDeclaredFeatures` is supported and enabled.
...
Re-enabling the feature and restarting the kubelet restores both behaviors.
The runtime-backed feature set is refreshed during the next successful
`updateRuntimeUp()` cycle. No data migration is needed. Pods that reached the
`Failed` phase while the feature was disabled remain failed.

###### Are there any tests for feature enablement/disablement?

The feature will be tested manually for Alpha. The main scenario requires an
in-place container runtime upgrade while the kubelet and existing workloads
remain running, which the current e2e test infrastructure does not support.
The manual test will cover the enabled, disabled, and re-enabled states and
will verify the behavior of both running and newly admitted Pods. The test
procedure and results will be documented before Beta.

### Rollout, Upgrade and Rollback Planning

###### How can a rollout or rollback fail? Can it impact already running workloads?

- Rollout/runtime changes: Dynamic discovery can add or remove declared features.
  A runtime downgrade picked up dynamically can remove features required by
  running Pods. These Pods are not terminated by rediscovery, but may encounter
  runtime errors. If re-discovery fails (e.g., CRI call timeout), the kubelet
  retains the last successfully discovered feature set.
- Rollback: Disabling `NodeDeclaredFeaturesRuntimeDiscovery` on the same kubelet
  version, or downgrading to a kubelet version that predates this feature gate,
  restores KEP-5328 behavior when `NodeDeclaredFeatures` remains enabled. On restart, any running
  Pod requiring a feature the node no longer declares can transition to `Failed`
  during re-admission.

###### What specific metrics should inform a rollback?

A persistent mismatch between the runtime capabilities and
`node.status.declaredFeatures` should trigger a rollback. For example, an
expected feature is still missing after two runtime status updates, or a
declared feature disappears even though the runtime continues to support it.
Operators can confirm the mismatch by checking the Node status together with
kubelet runtime status errors.

###### Were upgrade and rollback tested? Was the upgrade->downgrade->upgrade path tested?

Not yet. The upgrade -> downgrade -> upgrade path will be tested manually
during Alpha, and the results will be added here.
...
- [x] API .status
  - Other field: `node.status.declaredFeatures` is updated within ~10 seconds
    of a container runtime upgrade, without restarting the kubelet.
...
TODO before Beta: define the expected time for a runtime capability change to
appear in `node.status.declaredFeatures`, using data collected during Alpha.
...
- [ ] Other (treat as last resort)
  - Details: TODO before Beta. Define an SLI for the delay between runtime
    feature discovery and the corresponding Node status update.
...
Yes. Changes to the discovered feature set cause updates to
`node.status.declaredFeatures` through the existing Node status update path.
An unchanged feature set does not trigger an additional status write.
...
- Incompatible Pods may continue running after a feature they depend on is
  disabled while `NodeDeclaredFeaturesRuntimeDiscovery` remains enabled, or
  after a dynamically discovered runtime downgrade.
  - Detection: TODO before Beta.
  - Mitigations: Manually evict affected workloads.
  - Diagnostics: TODO before Beta.
  - Testing: TODO before Beta.
- CRI re-discovery may fail, leaving the last successfully discovered feature set
  in use even if runtime capabilities have changed.
  - Detection: TODO before Beta.
  - Mitigations: TODO before Beta.
  - Diagnostics: TODO before Beta.
  - Testing: TODO before Beta.

###### What steps should be taken if SLOs are not being met to determine the problem?

TODO before Beta: document how to compare the runtime features with
`node.status.declaredFeatures` and determine whether a failure happened during
discovery or while updating the Node status.
