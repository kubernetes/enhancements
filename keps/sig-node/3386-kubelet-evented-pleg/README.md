...
Generic PLEG pauses relists for a pod while `SyncPod` is running, so it does not repeat work or publish intermediate state. Requests received during this time are combined into one relist after `SyncPod` returns. Relists for other pods continue as usual. This behavior does not depend on the `EventedPLEG` feature gate.
...
Generic PLEG will expose a `kubelet_pleg_events_total` counter. Each time it generates a lifecycle event, it will increment the counter with a `relist_source` label: `post_sync`, `post_allocation`, `evented_pleg`, or `global`. This shows which relist source detected the change.

This metric will help show whether relists triggered by Evented PLEG detect changes before the next global relist. The current KEP only covers container termination. If we expand Evented PLEG to cover other lifecycle changes in the future, this metric can help us decide whether to reduce the global relist frequency. We would want to see changes detected by global relists approach zero before making that change. Reducing the global relist frequency remains a non-goal of this KEP.
...
Yes. Kubelet needs metrics for connection state, retries, and relist request outcomes. The new `kubelet_pleg_events_total` counter will also show which relist sources detect changes.
...
- v1.37: narrow Evented PLEG to the container-termination hint path
  - <https://github.com/kubernetes/kubernetes/pull/139262>
    ...
    Before this PR, Evented PLEG used CRI event payloads to update the pod cache and emit lifecycle events. It also increased Generic PLEG's polling interval and changed its configuration when the stream failed. This PR changed Evented PLEG to only request a pod relist when a container stops. Generic PLEG keeps its normal polling interval and is responsible for reading runtime state, updating the pod cache, and generating lifecycle events.
