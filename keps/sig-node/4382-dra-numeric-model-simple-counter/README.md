...
* Provide a simple migration path from extended resources to DRA. Driver
  developers don't need to invent a new resource model and don't need to
  implement a controller. They gain all of the other benefits of DRA
  (e.g. sharing of a resource allocation between containers or pods) and can
  enable others (configuration parameters, better validation of parameters).

* Usage for demonstrations and testing.

### Non-Goals

* Provide a model that can handle more complex hardware like GPUs.

## Proposal

The resource model is that each node contains a certain set of resource
instances. Each resource instance has its own independent set of items. A claim
can request "n" items, which must all be available in a single resource
instance. There are no restrictions on how a resource instance can be split up
among claims.
