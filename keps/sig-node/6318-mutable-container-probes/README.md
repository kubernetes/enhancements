...
The kubelet reconciles the probes in the latest PodSpec with its probe workers. It creates and removes workers as needed and updates existing workers without publishing an artificial probe failure. Results produced by an obsolete probe configuration or a previous container instance are not allowed to affect the current container.
...
The probe manager will reconcile the desired workers every time the kubelet processes an updated Pod. A worker is still identified by Pod UID, container name, and probe type. Reconciliation adds missing workers, updates retained workers, and stops workers for probes that were removed.

Workers use an immutable snapshot of probe configuration for each attempt. The manager invalidates attempts when their handler or timeout is replaced, when the worker is removed, or when the container ID changes. A late result from an invalid attempt is ignored even if cancellation did not stop the underlying probe in time.
...
| Probe added | Initialized | Determined by probe type | A worker is started |
| Probe removed | Removed | Recomputed as if no probe were configured | The worker and cached result are removed |
...
Removing a readiness probe makes a running container Ready when it is already Started. Removing a startup probe makes a running container Started and allows readiness and liveness probing to proceed; an existing readiness probe is scheduled promptly. Removing a liveness probe does not change Ready or Started. In all cases, cached results and stale in-flight results for the removed worker are discarded before Pod status is recomputed. Changes affecting more than one probe are reconciled as one batch to avoid publishing an intermediate status.
...
- [x] API `.status`
  ...
  - Other field:
- [x] Events
  - Event Reason: existing probe failure and container restart events

Users can also observe the behavior of the updated handler. For example, a readiness probe changed from a failing endpoint to a healthy one will result in the container becoming Ready after the configured threshold is reached.
...
The feature does not add calls made by Kubernetes components. Users and automation may issue ordinary Pod `UPDATE` or `PATCH` requests when they change a probe; their volume depends on how often workloads are changed. There is no periodic API traffic associated with the feature.
