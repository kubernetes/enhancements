...
This is not a tooling limitation. No runtime security agent, regardless of how well-instrumented, can solve this problem without upstream changes, because the identity is lost at the protocol level.

The Security Profiles Operator (SPO) currently works around this gap by using a mutating webhook to inject the API server's request UID as an environment variable (`SPO_EXEC_REQUEST_UID`) into exec sessions. While functional, this approach has limitations: environment variables live in userspace and can be read, modified, or cleared by the user inside the container. A proper upstream solution is needed to provide this identity through the Kubernetes architecture itself, making it available to any runtime security tool without requiring an external operator.
...
- Pass the audit request ID from the API server to the kubelet as part of the exec (and attach) request flow.
- Make it available to the kubelet so that it can be forwarded to the CRI runtime as part of the `ExecRequest`.
- Enable runtime security tooling to correlate in-container process activity with the Kubernetes user who initiated the exec session, using the propagated ID as a join key between the API server audit log and runtime observations.
...
- Modifying RBAC policies or admission control behavior. This KEP does not change who is allowed to exec into a pod. It only makes existing identity information available further down the stack.
- Providing a complete end-to-end audit logging solution for in-container activity. This KEP provides the identity plumbing, what runtime tooling does with that identity is outside the scope of this proposal.
...
Today, the exec request flows through three components:

1. API Server: The user's `kubectl exec` request arrives at the API server. The API server authenticates the caller, loads their identity into the request context, checks RBAC, and writes an audit log entry. It then calls the `ExecREST.Connect` handler, which resolves the target kubelet's URL and builds an upgrade-aware proxy handler. The proxy connects to the kubelet using the API server's own client certificate. The original caller's identity remains in the request context and is not forwarded.

2. Kubelet: The kubelet receives the proxied request and extracts the container ID, command, and stream flags. It calls the container runtime's `Exec` RPC via the CRI, passing an `ExecRequest` protobuf message with six fields: `container_id`, `cmd`, `tty`, `stdin`, `stdout`, `stderr`. There is no field for caller identity.

3. Container Runtime: The runtime (e.g., containerd, CRI-O) spawns the requested process inside the container's namespaces. It has no knowledge of who initiated the exec session.

#### Proposed changes

The changes would touch three points in this chain:

1. API Server to Kubelet: When the API server proxies the exec request to the kubelet, it would extract the identity information from the request context and include it as metadata, for example, as an HTTP header or as part of the request parameters.

2. Kubelet: The kubelet would be extended to read the identity information from the incoming request and include it when calling the runtime's `Exec` RPC.

3. CRI `ExecRequest`: An optional field would be added to the `ExecRequest` protobuf message to carry the identity information. Since protobuf is forward-compatible, runtimes that do not understand the new field would simply ignore it, maintaining backward compatibility.
...
This feature will be gated behind the `EXEC_REQUEST_ID` feature gate.
...
- [ ] Feature gate (also fill in values in `kep.yaml`)
  - Feature gate name: EXEC_REQUEST_ID
...
Yes, it can be disabled once it has been enabled.

###### What happens if we reenable the feature if it was previously rolled back?

New exec sessions will again include the `audit_request_id` in the CRI `ExecRequest`.
